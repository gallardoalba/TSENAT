...
patterns](https://raw.githubusercontent.com/gallardoalba/TSENAT/gh-pages/articles/TSENAT_files/figure-html/fig-1-isoform-diversity-profiles-1.png)
...
TSENAT includes three complementary vignettes that together provide a
complete guide to the package:
...
[Appendix
A](https://gallardoalba.github.io/TSENAT/articles/TSENAT_appendix_A.html)
validates TSENAT’s entropy and diversity computations against
[SplicingFactory](https://bioconductor.org/packages/SplicingFactory/),
an independent Bioconductor package. This cross-validation confirms that
Tsallis entropy values, diversity profiles, and the entropic index (q)
parameterization produce numerically equivalent results across both
implementations, ensuring computational correctness.

### Appendix B — Non-Parametric Validation

[Appendix
B](https://gallardoalba.github.io/TSENAT/articles/TSENAT_appendix_B.html)
provides non-parametric validation of the scale-adaptive interaction
test (SAIT) results using the Aligned Rank Transform (ART) via the
[ARTool](https://cran.r-project.org/package=ARTool) package. This
demonstrates concordance between the primary GAMM-based parametric
approach and a fully non-parametric alternative, strengthening
confidence in detected q × condition interactions.

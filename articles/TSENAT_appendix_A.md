...
This appendix characterizes the **mathematical relationship and endpoint
equivalence** between TSENAT’s transcript diversity measures (Shannon
entropy, Simpson index) and SplicingFactory’s implementations. We
benchmark both packages on TCGA BRCA RNA-seq data to demonstrate:

- Endpoint equivalence: TSENAT Tsallis q=1 recovers SplicingFactory
  Shannon (up to the documented normalization convention)
- Endpoint equivalence: TSENAT Tsallis q=2 recovers SplicingFactory
  Simpson (up to the documented normalization convention)
- Close empirical agreement under the tested
  dataset/filtering/normalization: same gene rankings, p-values, fold
  changes

The benchmark demonstrates agreement under the specific dataset,
filtering, normalization and statistical procedure tested here; it is
**not** a proof of general statistical equivalence across all datasets
and designs.
...
values provide empirical evidence of agreement in this benchmark, not
proof of equal statistical power in general.
...
1.  log2 fold changes: Normalization factor cancels in ratios:
    ...
    — this cancellation requires that $`K = 1 - 1/n`$ is the *same* for
    both conditions, which holds when $`n`$ is a fixed gene-level
    annotation universe but not necessarily when $`n`$ is an
    observed-support count
...
This appendix has characterized the **mathematical relationship and
endpoint equivalence** between TSENAT and SplicingFactory: at q=1 and
q=2, TSENAT recovers the corresponding Shannon and Gini–Simpson
quantities up to the documented normalization convention. The benchmark
shows close agreement under the tested dataset, filtering, normalization
and statistical procedure. It does not validate q\<1, q\>2, SAIT/paired
inference, bootstrap, divergence, q=0, or Salmon integration.
...
#> [13] TSENAT_0.99.35              SplicingFactory_1.18.0     

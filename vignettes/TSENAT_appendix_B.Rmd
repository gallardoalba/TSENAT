...
# Handle NA and NaN values properly
mean_effect <- mean(rrm_results$effect_size, na.rm = TRUE)
median_effect <- median(rrm_results$effect_size, na.rm = TRUE)
...
summary_stats <- data.frame(
  ...
  Value = c(
    nrow(rrm_results),
    sum(rrm_results$p_interaction < 0.05, na.rm = TRUE),
    sum(rrm_results$p_interaction < 0.05 & rrm_results$adj_p_interaction < 0.05, na.rm = TRUE),
    sum(is.na(rrm_results$p_interaction)),
    if (is.na(mean_effect) || !is.finite(mean_effect)) "Not available" else sprintf("%.1f%%", mean_effect * 100),
    if (is.na(median_effect) || !is.finite(median_effect)) "Not available" else sprintf("%.1f%%", median_effect * 100),
    sum(rrm_results$p_interaction < 0.05 & rrm_results$effect_size > 0.30, na.rm = TRUE),
    sprintf("%.1f%%", mean(rrm_results$model_converged, na.rm = TRUE) * 100)
  )
)
...
if ("p_interaction" %in% colnames(rrm_results)) {
  gam_sig <- rrm_results[!is.na(rrm_results$p_interaction), ]
  ...
  cat("Available columns:", paste(colnames(rrm_results), collapse=", "), "\n")
}
```

...
cat("DEBUG: pkg_root =", pkg_root, "\n")
devtools::load_all(pkg_root)
cat("DEBUG: find.package TSENAT =", find.package("TSENAT"), "\n")
cat("DEBUG: search path =", paste(search(), collapse = " | "), "\n")
cat("DEBUG: calculate_srh sources =", paste(getAnywhere("calculate_srh")$where, collapse = " | "), "\n")
cat("DEBUG: .store_srh_results sources =", paste(getAnywhere(".store_srh_results")$where, collapse = " | "), "\n")
...
cat("DEBUG: calculate_srh function: ", capture.output(getAnywhere("calculate_srh")), "\n")
cat("DEBUG: .store_srh_results function: ", capture.output(getAnywhere(".store_srh_results")), "\n")
analysis_rank <- calculate_srh(
    analysis,
    multicorr = "hochberg"
)

cat("DEBUG: analysis_rank class =", class(analysis_rank), "\n")
cat("DEBUG: slotNames(analysis_rank) =", paste(slotNames(analysis_rank), collapse = ", "), "\n")
cat("DEBUG: rank_test_results class =", class(analysis_rank@rank_test_results), "\n")
cat("DEBUG: rank_test_results length =", length(analysis_rank@rank_test_results), "\n")
cat("DEBUG: rank_test_results names =", paste(names(analysis_rank@rank_test_results), collapse = ", "), "\n")
if (!is.null(analysis_rank@rank_test_results) && length(analysis_rank@rank_test_results) > 0) {
    cat("DEBUG: rank_test_results[1] structure:\n")
    print(str(analysis_rank@rank_test_results[[1]], max.level = 1))
}
if (is.null(analysis_rank@rank_test_results) || length(analysis_rank@rank_test_results) == 0) {
    stop("No rank test results were stored in analysis_rank@rank_test_results after calculate_srh().")
}

# Extract ALL results (no filtering) for summary statistics
srh_results_all <- results(analysis_rank, type = "rank_test", rankBy = "pvalue")

# Extract top significant genes for display
srh_results <- results(analysis_rank, type = "rank_test", rankBy = "pvalue", n = 20, filterFDR = 0.05)
...
knitr::kable(rank_summary_stats, caption = "**Supplementary Table 15 | Scheirer-Ray-Hare Test Results Summary.** Rank-based nonparametric test of *q* × condition interactions. Compare with GAMM results (main vignette Table 3) to assess parametric vs. nonparametric method concordance.", label = "srh-test-summary", align = c("l", "r"))
...
cat("DEBUG: before calculate_concordance, rank_test_results class =", class(analysis_rank@rank_test_results), "\n")
cat("DEBUG: before calculate_concordance, rank_test_results names =", paste(names(analysis_rank@rank_test_results), collapse = ", "), "\n")
if (is.null(analysis_rank@rank_test_results) || length(analysis_rank@rank_test_results) == 0) {
    stop("Cannot run calculate_concordance(): analysis_rank@rank_test_results is empty. Ensure calculate_srh() completed successfully.")
}

...
transcripts with a sufficient minimum expression level. Expression 
estimates of transcript isoforms with zero or low expression might 
be highly variable. For more details on the effect of transcript 
isoform prefiltering on differential transcript usage, see 
...
In the previos code-block a **pseudocount** was used to address
zero values in gene expression datasets. By adding a 
pseudocount,zeros can be treated as very low values, enabling
more meaningful analyses. This approach prevents undefined 
`\(\log (0)\)` values, stabilizes variance for low-abundance 
features, and ensures robust differential expression analysis.
Using a small pseudocount, such as 1e-6, maintains data 
...
studies when per-term inference is required (for example, the
`q:group` interaction).
...
```{r hapln3-gene-qprofile, fig.width=8, fig.height=5}
...
```{r colia2-gene-qprofile, fig.width=8, fig.height=5}
# Plot q-curve profile for top linear-model gene
    # Prefer GFPT1 if present; otherwise use top LM hit
...
```{r pi16-gene-qprofile, fig.width=8, fig.height=5}
# Plot q-curve profile for top linear-model gene
    # Prefer GFPT1 if present; otherwise use top LM hit
...
```{r tsallis-q-curve, fig.width=12, fig.height=6}

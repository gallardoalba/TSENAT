...
TSENAT (Tsallis Entropy Analysis Toolbox) quantifies **isoform complexity** in RNA-seq data using Tsallis entropy-a scale-dependent diversity measure distinct from abundance-focused tools (DESeq2, edgeR) and differential transcript usage packages (DRIMseq, SplicingFactory).

## Motivation and Bioconductor Contribution

Common RNA-seq tools focus on either total gene abundance changes or individual transcript usage shifts, yet genes often remodel their isoform landscapes without changing overall expression-a phenomenon missed by these approaches. TSENAT quantifies this isoform complexity directly via Tsallis entropy, a scale-dependent diversity framework tuned by parameter `q`. By sliding `q` across scales, it makes possble to zoom between rare variants (low `q`) and dominant isoforms (high `q`), capturing biological signal invisible to abundance- or proportion-based summaries.

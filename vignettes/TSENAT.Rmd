---
title: "TSENAT: Tsallis entropy workflow"
author: "TSENAT developers"
date: "`r Sys.Date()`"
output:
...
Many genes express multiple isoforms whose relative abundances can be multimodal or 
heavy-tailed. Standard mean-based comparisons may miss changes in isoform dominance 
or heterogeneity. Tsallis entropy provides a tunable way to weight rare versus dominant
isoforms by varying `q`, producing a "q-curve" that reveals scale-dependent diversity 
differences.
...
- Limit $q\to 1$: Shannon entropy, \(\lim_{q\to1} S_q(p) = -\sum_i p_i\log p_i\).
...
Read optional sample metadata from `coldata.tsv` and preview it if present.

```{r read-coldata, echo=TRUE}
...
If you prefer to add a small pseudocount instead of filtering transcripts,
do so explicitly and document the choice. Example:

```{r pseudocount-example, echo=TRUE}
...
Here we will use Wilcoxon, which is the adecuate option for speed and when 
sample sizes support asymptotic approximations; use permutation when small-sample 
accuracy or exact control of the null distribution is important.
...
- The MA plot shows the relationship between mean expression and log-fold change,
...
if (all(c("mean_difference", "adjusted_p_values") %in% colnames(res))) {
  res$label <- apply(
    res[, c("mean_difference", "adjusted_p_values")], 1,
    function(x) {
      ifelse(abs(as.numeric(x[1])) >= 0.1 & as.numeric(x[2]) < 0.05,
...
if (is.data.frame(res) && nrow(res) > 0) {
  ...
  if (is.null(rownames(counts_for_plot))) {
    message(
      "Skipping transcript plots: `readcounts` does not have transcript rownames.",
...
It allows to compare values across `q` for the same gene to see whether diversity
differences are scale-dependent (e.g., significant at `q=0.1` but not at
...
With the q-sequence we can produces a q-curve per sample and gene. These curves show how diversity 
emphasis shifts from rare to dominant isoforms as `q` increases and form the basis for interaction 
tests. The q-curve shows entropy as a function of `q`. Diverging curves between groups 
indicate scale-dependent diversity differences. Separation at low `q` implies differences 
in rare isoforms; separation at high `q` signals differences in dominant isoforms.

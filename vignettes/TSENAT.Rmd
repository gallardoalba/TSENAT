...
diversity <- results(analysis, type = "diversity", display_table = TRUE, n_genes = 4, sample = "SRR14800481")
print(diversity)
```
...
tables_result <- results(analysis, type = "switching_tables", display_table = TRUE)
```
...
```{r extract-divergence-results, echo=TRUE}
# Extract top 6 genes by statistical significance
top_genes_result <- results(
    analysis, 
    type = "effect_sizes_divergence",
...
# Display the result (data.frame with effect size information)
print(top_genes_result)
```

```{r divergence-results-table, results='markup', echo = FALSE}
# The display_table=TRUE in the results() call above automatically formatted and displayed
# the top 6 genes with their effect sizes and q-patterns in a human-readable format
cat("Effect sizes reveal which genes show the most information-theoretic separation across the q-spectrum between paired treatment groups.\n")
cat("Genes with Tsallis divergence D > 0.1 show substantial divergence, indicating fundamental differences in entropy distributions between conditions.\n")
```

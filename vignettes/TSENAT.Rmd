...
se <- filter_se(se, min_count = 5L, min_samples = 5L, verbose = TRUE)
```
...
# Aggregate transcript-level read counts stored in `se` to gene-level by summing transcripts
# Use the assay from the `SummarizedExperiment` and gene mapping in `rowData`
readcounts_mat <- SummarizedExperiment::assay(se)
genes_vec <- SummarizedExperiment::rowData(se)$genes

...
**Delta influence** (from jackknife resampling [@S115]) quantifies *how each transcript's relative importance changes between conditions*, weighted by stability across bootstrap iterations (positive = more influential in first condition; negative = second condition).
...
To retrieve the effect sizes and q-spectrum patterns programmatically, use the `results()` accessor:

```{r retrieve-effect-sizes-results}
# Extract effect sizes and patterns using the unified results() accessor
effect_sizes_df <- results(analysis, type = "effect_sizes_divergence")

# Display top 6 genes by statistical significance
if (!is.null(effect_sizes_df) && is.data.frame(effect_sizes_df)) {
    # Select key columns for display
    key_cols <- c("gene", "gene_name", "adj_p_interaction", "Mean_Divergence", 
                  "per_q_pattern", "div_rare_median", "div_abundant_median", "q_ratio")
    
    # Keep only columns that exist
    available_cols <- key_cols[key_cols %in% colnames(effect_sizes_df)]
    display_df <- effect_sizes_df[, available_cols, drop = FALSE]
    
    # Sort by p-value and show top 6
    display_df <- display_df[order(display_df$adj_p_interaction, na.last = TRUE), ]
    display_df <- head(display_df, 6)
    
    cat("Top 6 genes by effect size and statistical significance:\n")
    print(display_df)
}
```
...
**Table 5 | Top genes by linear model significance with effect sizes and q-spectrum patterns.** 
Ranked by statistical significance (ascending P-values, Benjamini-Hochberg multiple comparison correction). 
Columns: **Gene** - gene identifier; **Gene Name** - standard gene symbol; **P-value** - unadjusted p-value 
for q×condition interaction; **Adj. P-value** - multiple-comparison-corrected p-value; **Mean Effect Size** - 
mean Tsallis divergence D across the full q-spectrum (0 to 2.0), measuring information-theoretic separation 
between conditions; **Q-Pattern** - classification from hierarchical clustering of per-q divergence values 
(Rare-driven/Balanced/Abundant-driven); **D_rare** - median divergence for rare isoforms (q < 1.0); 
**D_abundant** - median divergence for abundant isoforms (q > 1.0); **Ratio** - D_rare / D_abundant 
(>1.3 indicates rare-driven, <0.77 indicates abundant-driven, 0.77-1.3 indicates balanced). Genes meeting 
significance threshold (q < 0.05) with largest divergence values indicate the most robust transcriptomic 
reorganization between treatment groups across multiple diversity scales.

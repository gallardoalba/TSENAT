...
### Reproducibility: downloading raw TCGA files and verifying the built dataset

The script `data-raw/tcga_brca_luma_dataset.R` implements the download and processing steps used to create the packaged example dataset. To reproduce the raw-file download step you can run the following code:

```{r repro-raw, eval = FALSE}
# Query and download isoform quantification files for TCGA-BRCA
...
# Or run the packaged script which automates the workflow
# system("Rscript data-raw/tcga_brca_luma_dataset.R")
```

After running the script you can compute an MD5 checksum for the produced `RData` and compare it to the released package checksum to confirm reproducibility:

```{r check-md5, eval = FALSE}
# Compute md5 of produced file
tools::md5sum("data/tcga_brca_luma_dataset.RData")
# Or use the helper script to write a checksum file to inst/extdata:
# system("Rscript data-raw/checksum_tcga_brca_luma_dataset.R")
```

Expected MD5 for the packaged dataset: `7583a6392568f2274b33b1cc4ab3b542`.

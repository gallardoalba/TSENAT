...
An example dataset is included for demonstration. It consists of transcript-level 
read counts from 20 patients with Luminal A type breast cancer (TCGA BRCA project), 
with 40 samples total (21 normal + 21 tumor). The data includes:

- **Read count matrix**: 1,100 transcripts × 40 samples
- **GFF3 annotation file**: GENCODE-format GFF3.gz for transcript-to-gene mapping
- **Sample metadata**: Phenotype classification (Normal vs Tumor)

For more details on the dataset and annotation files, 
see `inst/extdata/README.md` in the package.
...
Now will use a GFF3.gz annotation file for performing the transcript-to-gene 
mapping.
...
TSENAT uses the **GFF3 (Generic Feature Format version 3)** standard for 
transcript-to-gene annotations. This is the recommended and primary format for 
providing annotation data. GFF3 is comprehensive and widely used in bioinformatics 
(GENCODE, Ensembl, RefSeq).

The `build_se()` function requires a GFF3 file (either uncompressed `.gff3` or 
compressed `.gff3.gz`) and automatically extracts transcript-to-gene mappings 
from the `transcript` or `mRNA` features.

### GFF3 File Requirements

- Must contain `transcript` or `mRNA` features
- Each transcript feature must have `ID` and `Parent` attributes
- Parent attribute should reference the gene ID
- Recommended: Use gzip compression (`.gff3.gz`) for large files

### Usage Example

In TSENAT workflows, you provide the GFF3 file path to `build_se()`:

```{r gff3-usage-example, eval = FALSE}
# Load transcript-to-gene mapping from GFF3.gz file
gff3_path <- system.file("extdata", "gencode_subset.gff3.gz", package = "TSENAT")
se <- build_se(readcounts, gff3_path)
```

The example dataset includes a reference annotation file 
(`gencode_subset.gff3.gz`) demonstrating the required GFF3 format.

For comprehensive documentation on annotation format specifications and data generation 
workflow, see `inst/extdata/README.md` in the package.

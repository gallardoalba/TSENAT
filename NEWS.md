...
TSENAT computes Tsallis entropy (S_q) and related Hill numbers (D_q) to quantify isoform/transcript-level diversity per gene. The package accepts matrices, tximport-style lists, and SummarizedExperiment objects, returns per-gene diversity assays, and provides plotting and simple differential-analysis helpers. A vignette demonstrates a full workflow on a TCGA Luminal A example.

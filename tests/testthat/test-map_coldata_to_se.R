...
test_that("Reversed coldata reorders SE to follow coldata", {
    ...
    mapped_base <- sub("_q=.*", "", colnames(SummarizedExperiment::assay(se2)))
    expect_equal(mapped_base[seq_len(nrow(coldata_rev))], as.character(coldata_rev$Sample))
})
...
test_that("Missing sample in coldata triggers error (unpaired)", {
    ...
    expect_error(map_coldata_to_se(se, coldata_missing, paired = TRUE), "Unpaired samples found in coldata")
})
...
test_that("Missing sample in coldata does not error when paired is FALSE", {
    ...
    expect_equal(ncol(SummarizedExperiment::assay(se2)), ncol(SummarizedExperiment::assay(se)))
})
test_that("Extra sample in coldata triggers error (unpaired)", {
    ...
    extra <- data.frame(Sample = "FAKE_SAMPLE_N", Condition = "Normal", stringsAsFactors = FALSE)
    ...
    expect_error(map_coldata_to_se(se, coldata_extra, paired = TRUE), "Unpaired samples found in coldata")
})
...
test_that("Extra sample in coldata does not error when paired is FALSE", {
    ...
    expect_equal(ncol(SummarizedExperiment::assay(se2)), length(cols))
})
test_that("Case mismatch in coldata avoids name-based mapping (case-sensitive)", {
    ...
    mapped_base <- sub("_q=.*", "", colnames(SummarizedExperiment::assay(se2)))
    # None of the mapped bases should match exact lowercase coldata sample names
    expect_equal(sum(mapped_base %in% coldata_case$Sample), 0)
    # sample_type should still be filled via inference
    expect_true(all(!is.na(SummarizedExperiment::colData(se2)$sample_type)))
})
...
test_that("SE with extra sample keeps extra and marks it as unmatched in coldata", {
    ...
    se2 <- map_coldata_to_se(se, coldata)
    mapped_base <- sub("_q=.*", "", colnames(SummarizedExperiment::assay(se2)))
    expect_true("EXTRA_SAMPLE" %in% mapped_base)
    expect_equal(sum(mapped_base %in% as.character(coldata$Sample)), nrow(coldata))
})
...
test_that("Non-data.frame coldata is ignored and SE remains unchanged", {
    ...
    named_vec <- setNames(as.character(coldata$Condition), as.character(coldata$Sample))
    sample_names <- as.character(coldata$Sample)
    cols <- paste0(sample_names, "_q=0.1")
    mat <- matrix(runif(length(cols)), nrow = 1)
    colnames(mat) <- cols
    ...
    expect_equal(colnames(SummarizedExperiment::assay(se2)), colnames(SummarizedExperiment::assay(se)))
...
test_that("colData rownames and sample_base are aligned to assay columns", {
    coldata <- utils::read.delim(system.file("extdata/coldata.tsv", package = "TSENAT"), stringsAsFactors = FALSE)
    sample_names <- as.character(coldata$Sample)
    cols <- paste0(sample_names, "_q=0.1")
    mat <- matrix(runif(length(cols)), nrow = 1)
    colnames(mat) <- cols
    ...
    expect_equal(as.character(cd$sample_base), sub("_q=.*", "", colnames(SummarizedExperiment::assay(se2))))
})

test_that("Missing samples are inferred via infer_sample_group when possible", {
    coldata <- utils::read.delim(system.file("extdata/coldata.tsv", package = "TSENAT"), stringsAsFactors = FALSE)
    # remove one sample so it must be inferred
    removed <- as.character(coldata$Sample[3])
    coldata_missing <- coldata[-3, , drop = FALSE]
    sample_names <- as.character(coldata$Sample)
    cols <- paste0(sample_names, "_q=0.1")
    mat <- matrix(runif(length(cols)), nrow = 1)
    ...
    se2 <- map_coldata_to_se(se, coldata_missing)
    base_names <- sub("_q=.*", "", colnames(SummarizedExperiment::assay(se2)))
    idx <- which(base_names == removed)
    expect_length(idx, 1)
    inferred <- infer_sample_group(removed)
    # the sample_type for the missing sample should equal the inferred label
    expect_equal(as.character(SummarizedExperiment::colData(se2)$sample_type[idx]), inferred)
})

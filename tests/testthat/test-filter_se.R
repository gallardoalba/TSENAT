...
test_that("filter_se_by_interaction_effect_size filters by eta2 threshold", {
    ...
    # Create interaction results with different effect sizes
    interaction_results <- data.frame(
        gene = paste0("gene", 1:6),
        effect_size_eta2 = c(0.05, 0.03, 0.015, 0.01, 0.005, 0.001),
        pvalue = 0.01
    )
    ...
    expect_equal(nrow(res), 2)
    expect_true(all(c("gene1", "gene2") %in% rownames(res)))
...
test_that("filter_se_by_interaction_effect_size handles NA effect sizes", {
    ...
    # Some genes have NA effect sizes
    interaction_results <- data.frame(
        gene = paste0("gene", 1:6),
        effect_size_eta2 = c(0.05, NA, 0.025, NA, 0.001, 0.003)
    )
...
test_that("filter_se_by_interaction_effect_size works with eta2 column name", {
    ...
    # Use 'eta2' column name instead of 'effect_size_eta2'
    interaction_results <- data.frame(
        gene = paste0("gene", 1:6),
...
test_that("filter_se_by_interaction_effect_size warns when all genes filtered", {
    ...
    # All effect sizes below threshold
    interaction_results <- data.frame(
        gene = paste0("gene", 1:6),
        eta2 = c(0.001, 0.002, 0.001, 0.0005, 0.001, 0.0008)
    )
...
test_that("filter_se_by_interaction_effect_size preserves metadata", {
    ...
    se <- SummarizedExperiment(assays = list(counts = mat))
    S4Vectors::metadata(se)$original_count <- 6
    ...
    # Effect size filter info should be added
    expect_true(!is.null(S4Vectors::metadata(res)$effect_size_filter))
    filter_info <- S4Vectors::metadata(res)$effect_size_filter
    expect_equal(filter_info$threshold, 0.02)
    expect_equal(filter_info$genes_before, 6)
    # eta2 > 0.02: only genes 1 (0.05) and 2 (0.03) pass
    expect_equal(filter_info$genes_after, 2)
})
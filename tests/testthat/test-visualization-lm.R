...
context("plot_lm_gam_s4: S4 wrapper for GAM-based interaction visualization")
...
test_that("plot_lm_gam_s4: requires LM interaction results", {
  ...
  expect_error(
    TSENAT::plot_lm_gam_s4(analysis),
    regex = "No LM interaction results"
  )
})

test_that("plot_lm_gam_s4: requires diversity results", {
  skip_if_not_installed("SummarizedExperiment")
...
test_that("plot_lm_gam_s4: auto-detects condition column", {
  skip_if_not_installed("SummarizedExperiment")
...
test_that("plot_lm_gam_s4: handles n_top parameter", {
  skip_if_not_installed("SummarizedExperiment")
  ...
  for (n in c(1, 3, 5)) {
    result <- tryCatch(
      TSENAT::plot_lm_gam_s4(analysis, n_top = n),
      error = function(e) NULL
    )
    expect_true(is.null(result) || inherits(result, "ggplot") || is.list(result))
  }

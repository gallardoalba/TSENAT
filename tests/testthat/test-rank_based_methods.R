...
# Test Suite: recommend_q_range()
test_that("recommend_q_range suggests FULL spectrum for strong interactions", {
  # Simulate results: 10% strongly q-dependent genes
  interaction_results <- data.frame(
    gene = paste0("Gene", 1:100),
    p_value = c(rep(0.01, 10), rep(0.10, 90)),  # 10 significant
    effect_size_eta2 = c(rep(0.15, 10), rep(0.02, 90)),  # 10 large effects
    f_statistic = c(rep(8.0, 10), rep(2.0, 90)),
    interaction_class = c(rep("Strongly q-dependent", 10), rep("Robust across q", 90)),
    stringsAsFactors = FALSE
  )
  
  recommendation <- recommend_q_range(interaction_results)
  
  expect_match(recommendation$recommendation, "FULL q-spectrum")
  expect_equal(length(recommendation$suggested_q_values), 6)
  expect_equal(recommendation$suggested_q_values, c(0.1, 0.5, 1.0, 1.5, 2.0, 2.5))
})

test_that("recommend_q_range suggests STANDARD range for moderate interactions", {
  # Simulate results: 25% moderately q-dependent, 75% robust (below 70% threshold for FOCUSED)
  # But need moderate > 10% for STANDARD to trigger (since robust check comes second)
  interaction_results <- data.frame(
    gene = paste0("Gene", 1:100),
    p_value = c(rep(0.03, 30), rep(0.10, 70)),
    effect_size_eta2 = c(rep(0.05, 30), rep(0.02, 70)),
    f_statistic = c(rep(3.5, 30), rep(2.0, 70)),
    interaction_class = c(rep("Moderately q-dependent", 30), rep("Robust across q", 70)),
    stringsAsFactors = FALSE
  )
  
  recommendation <- recommend_q_range(interaction_results)
  
  expect_match(recommendation$recommendation, "STANDARD q-range")
  expect_equal(recommendation$suggested_q_values, c(0.5, 1.0, 1.5, 2.0))
})

test_that("recommend_q_range suggests FOCUSED range for robust genes", {
  # Simulate results: mostly robust genes
  interaction_results <- data.frame(
    gene = paste0("Gene", 1:100),
    p_value = c(rep(0.15, 75), rep(0.03, 25)),
    effect_size_eta2 = c(rep(0.01, 75), rep(0.06, 25)),
    f_statistic = c(rep(1.5, 75), rep(3.0, 25)),
    interaction_class = c(rep("Robust across q", 75), rep("Moderately q-dependent", 25)),
    stringsAsFactors = FALSE
  )
  
  recommendation <- recommend_q_range(interaction_results)
  
  expect_match(recommendation$recommendation, "FOCUSED|FIXED")
})

test_that("recommend_q_range returns list with required elements", {
  interaction_results <- data.frame(
    gene = paste0("Gene", 1:50),
    p_value = rnorm(50, mean = 0.10, sd = 0.05),
    effect_size_eta2 = rnorm(50, mean = 0.05, sd = 0.03),
    f_statistic = rnorm(50, mean = 3.0, sd = 1.0),
    interaction_class = rep("Robust across q", 50),
    stringsAsFactors = FALSE
  )
  ...
  expect_true(all(c("recommendation", "rationale", "robust_pct", "moderate_pct", 
                     "strong_pct", "suggested_q_values", "sample_sizes") %in% 
                   names(recommendation)))
})

test_that("recommend_q_range percentages sum to 1.0", {
  interaction_results <- data.frame(
    gene = paste0("Gene", 1:100),
    p_value = c(rep(0.10, 70), rep(0.03, 20), rep(0.01, 10)),
    effect_size_eta2 = c(rep(0.02, 70), rep(0.05, 20), rep(0.15, 10)),
    f_statistic = c(rep(2.0, 70), rep(3.0, 20), rep(7.0, 10)),
    interaction_class = c(rep("Robust across q", 70), 
                          rep("Moderately q-dependent", 20),
                          rep("Strongly q-dependent", 10)),
    stringsAsFactors = FALSE
  )
  
  recommendation <- recommend_q_range(interaction_results)
  
  total_pct <- recommendation$robust_pct + recommendation$moderate_pct + recommendation$strong_pct
  expect_equal(total_pct, 1.0, tolerance = 0.001)
})

test_that("recommend_q_range sample_sizes total matches nrow", {
  n_genes <- 42
  interaction_results <- data.frame(
    gene = paste0("Gene", 1:n_genes),
    p_value = rnorm(n_genes, mean = 0.10, sd = 0.05),
    effect_size_eta2 = rnorm(n_genes, mean = 0.05, sd = 0.03),
    f_statistic = rnorm(n_genes, mean = 3.0, sd = 1.0),
    interaction_class = rep("Robust across q", n_genes),
    stringsAsFactors = FALSE
  )
  
  recommendation <- recommend_q_range(interaction_results)
  
  total_samples <- (recommendation$sample_sizes$robust + 
                    recommendation$sample_sizes$moderate + 
                    recommendation$sample_sizes$strong)
  expect_equal(total_samples, n_genes)
})

test_that("recommend_q_range respects custom thresholds", {
  # 8% strongly q-dependent (above default 5% but below 10%)
  interaction_results <- data.frame(
    gene = paste0("Gene", 1:100),
    p_value = c(rep(0.01, 8), rep(0.10, 92)),
    effect_size_eta2 = c(rep(0.15, 8), rep(0.02, 92)),
    f_statistic = c(rep(8.0, 8), rep(2.0, 92)),
    interaction_class = c(rep("Strongly q-dependent", 8), rep("Robust across q", 92)),
    stringsAsFactors = FALSE
  )
  
  # With default threshold (5%)
  rec_default <- recommend_q_range(interaction_results, strong_threshold = 0.05)
  expect_match(rec_default$recommendation, "FULL q-spectrum")
  
  # With higher threshold (10%)
  rec_higher <- recommend_q_range(interaction_results, strong_threshold = 0.10)
  expect_match(rec_higher$recommendation, "STANDARD|FOCUSED")
})

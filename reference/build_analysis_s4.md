...
- tpm:
  ...
  build_analysis_s4(), subsequent filter_analysis_s4() calls will fail
...
- effective_length:
  ...
  If not provided to build_analysis_s4(), the data will not be stored
  for later use in length-normalized calculations.
...
- config:
  ...
  FIRST, then pass to `build_analysis_s4()` at object construction time.
...
- @diversity_results:
  ...
  Empty list (populated by calculate_diversity_s4())

- @divergence_results:

  Empty list (populated by calculate_divergence_s4())

- @lm_results:

  Empty list (populated by calculate_lm_interaction_s4())
...
#> Created: 2026-04-09 01:12:29.518352
#> 
print(dim(analysis))
#> NULL

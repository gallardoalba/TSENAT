...
#' 'lmm': linear mixed models with AR(1) covariance for q-ordered
#' measurements (requires nlme).
#' 'gam': generalized additive models with flexible smoothing (requires mgcv).
#' 'fpca': functional principal components analysis for curve data. Respects
#' q-value ordering
#' by treating q-values as ordered measurements in a functional data
#' framework (Papers S168-S171).
#' PCA on ordered curves implicitly captures AR(1) correlation structure.
#' 'gee': generalized estimating equations for clustered/paired data
#' (requires geepack).
...
#' requires lme4
#' which does not support AR(1) covariance structures). This parameter is
#' ignored for LMM.
...
#' used in all methods. See `multicorr` for the primary multiple testing
#' correction.
...
#' 'exchangeable': Compound symmetry (appropriate when measurements are
#' unordered).
...
#' bias correction
#' for GEE with small number of clusters (default: TRUE). When TRUE and the
#' number of
#' clusters is less than 20, uses t-distribution instead of normal
#' distribution for p-value
#' computation, which maintains Type I error rate for small sample GEE
#' analyses. Reference:
...
#' one of
#' \code{c('pca',  'lasso',  'elasticnet')} (default:  'pca').
#'  'pca' uses classical PCA
#' extracting the first principal component (PC1). 'lasso' uses L1-penalized
#' logistic
#' regression with cross-validation to select important q-values
#' (References: Friedman et al. 2010,
#' Bloch 2020). 'elasticnet' uses elastic net (L1+L2 penalty, alpha=0.5) for
#' improved stability
#' (References: Friedman et al. 2010, Byliskii 2015). Regularization methods
#' can provide
#' higher statistical power than PC1 by automatically selecting informative
#' q-values. This
...
#' interaction
#' p-values from linear models naturally exhibit AR(1) correlation for
#' different q-values
#' of the same gene (Papers S168-S175). This parameter selects the primary
#' multiple testing
#' correction method:
...
#' 'westfall-young': True Westfall-Young permutation procedure (FWER <=
#' alpha). Uses resampling
#' to empirically control FWER by tracking the minima across all tests. More
#' powerful than
#' Hochberg under dependence but computationally expensive (refits LMM for
#' each permutation).
...
#' Higher values improve accuracy of empirical null distribution but
...
#' @param storey Logical; whether to apply Storey's adaptive FDR ?0
#' estimation after the
#' selected multicorr method (default: FALSE). When TRUE, adapts the error
#' threshold based
#' on estimated proportion of true null hypotheses, increasing power when
#' many true signals
#' are present. Can be applied to any multicorr method. Computationally
#' light enhancement.
...
#' selection for GAM method
#' (default: TRUE). When TRUE, automatically adjusts the number of basis
#' functions (k) per gene
#' based on entropy curve complexity, measured as coefficient of variation
#' of slopes across
#' ordered q-values. Simple curves get fewer knots (min=2), complex
#' non-monotonic curves get
#' more knots (max=10), improving model fit efficiency. When FALSE, uses
#' fixed knot selection
#' based on number of unique q-values. Reference: Wood (2017) Section 4.1.5
#' Basis dimension.
...
#' \itemize{
#'   \item `$results`:  The standard results data.
#' frame (same as returned when  FALSE)
#'   \item `$model_data`:  A list containing model metadata (method,
#'  q-values,  sample info,  
#' test configuration, genes analyzed, etc.) useful for generating
#' diagnostic plots and
#' understanding model structure. Can be passed to plotting functions for
#' visualization.
#' }
#' When FALSE, returns only the results data.frame (backward compatible with
#' existing code).
#' This enables users to access comprehensive model information for
#' diagnostics and visualization
#' while maintaining full backward compatibility.
#' @return When `return_model_data = FALSE` (default): A data.frame with
#' columns `gene`,
#' `p_interaction`, and `adj_p_interaction`, ordered by ascending
#' `p_interaction`.
#' 
...
#'  sample names,  test configuration,
...
#' 'control', 'treatment').
#' Default: 'condition'. Map your grouping variable into this column before
#' calling TSENAT functions.}
...
#' interaction
#' models. Chapter 8.2.7 covers testing hypotheses in multiple linear
#' regression.
...
#' methodology,  smooth basis selection,  and  model comparison for 
#' \code{method='gam'}.
#' 
#' Liang, K. Y., & Zeger, S. L. (1986). Longitudinal data analysis using 
#' generalized linear models. \emph{Biometrika}, 73(1), 13-22. Foundational
#' paper
#' introducing Generalized Estimating Equations (GEE) for clustered and
#' repeated
...
#' the Royal
#' Statistical Society}, Series B, 57, 289-300. Used for multiple testing
#' correction
#' via \code{p.adjust(..., method='BH')}.
#' 
#' Hochberg, Y. (1988). A sharper Bonferroni procedure for multiple tests of 
#' significance. \emph{Biometrika}, 75(4), 800-802. Stepup procedure for
#' FWER control
#' used in multicorr='westfall-young' option. More powerful than Bonferroni.
#' 
#' Benjamini, Y., & Yekutieli, D. (2001). The control of the false discovery
#' rate
#' in multiple testing under dependency. \emph{Annals of Statistics}, 29(4),
#' 1165-1188.

...
.label_shuffling <- function(x, samples, control, method, randomizations = 100, pcorr = "BH",
    ...
    perm_mat <- .generate_permutation_matrix(x, samples, control, method, randomizations,
    ...
    .format_pvalue_output(raw_p_values, adjusted_p_values, log2_fc, effect_stats,
        group_means)
}
...
#'
#' @noRd
.validate_label_shuffling_inputs <- function(samples, control, pairs, n_samples,
    paired) {
    ...
    if (length(unique_groups) != 2) {
        stop(".label_shuffling() requires exactly 2 sample groups; found ", length(unique_groups),
            ": ", paste(unique_groups, collapse = ", "), call. = FALSE)
    }

    # Check for even number of samples when paired design
    if (isTRUE(paired) && (n_samples%%2 != 0)) {
        stop("Paired permutation requires an even number of samples", call. = FALSE)
    }
...
.compute_pseudocount <- function(fc_result) {
    ...
    if (length(pos_vals) > 0)
        min(pos_vals, na.rm = TRUE)/2 else 1e-06
}
...
.generate_permutation_matrix <- function(x, samples, control, method, randomizations,
    ...
    if (isTRUE(paired) && !is.null(pairs)) {
        perm_mat <- .generate_paired_permutations(x, samples, control, case_group,
            method, randomizations, pairs, paired_method, pseudocount_val, robust_loss_type,
            robust_scale_method)
    } else {
...
.generate_paired_permutations <- function(x, samples, control, case_group, method,
    ...
    # OPTIMIZATION: Pre-generate all random decisions for all randomizations at
    # once This avoids repeated sample() calls inside the loop
    if (paired_method == "swap") {
        # For swap: generate (randomizations × n_pairs) boolean matrix
        swap_matrix <- matrix(sample(c(TRUE, FALSE), size = randomizations * n_pairs,
            replace = TRUE), nrow = randomizations, ncol = n_pairs)
...
.compute_pvalues_from_permutations <- function(log2_fc, perm_mat, nthreads) {
    # OPTIMIZATION: Pre-compute row counts of non-NA values to avoid repeated
    # filtering Each element [i] = count of non-NA permutation values for
    # feature i
...
.compute_all_effect_sizes <- function(x, samples, pairs, groups, nthreads) {
    ...
    list(U = vapply(effect_sizes, "[", i = "U", FUN.VALUE = numeric(1)), r = vapply(effect_sizes,
        "[", i = "r", FUN.VALUE = numeric(1)))
}
...
.wilcox_effect_sizes_unpaired <- function(x, feature_idx, samples, groups) {
    ...
    wt <- wilcox.test(x[feature_idx, g1_idx], x[feature_idx, g2_idx], paired = FALSE,
        exact = FALSE)
    U <- as.numeric(wt$statistic)
    n1 <- length(g1_idx)
    n2 <- length(g2_idx)
...
#'
#' @noRd
.format_pvalue_output <- function(raw_p_values, adjusted_p_values, log2_fc, effect_stats,
    group_means) {

    out <- data.frame(pvalue = raw_p_values, padj = adjusted_p_values, log2FC = log2_fc,
        U = effect_stats$U, r = effect_stats$r, group_means, check.names = FALSE,
        stringsAsFactors = FALSE)

    # Extract group names by removing method suffix (e.g., 'Normal_mean' ->
    # 'Normal')

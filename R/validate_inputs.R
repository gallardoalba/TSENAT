...
# ============================================================================
# Purpose: Centralized validation helpers for plot functions Ensures consistent
# error messages and graceful handling of invalid inputs
# ============================================================================
...
#' @noRd
.validate_se <- function(se, assay_name = NULL, min_genes = 1, min_samples = 1, allow_null = FALSE) {
    # Check for NULL
    if (is.null(se)) {
        if (allow_null)
    ...
    if (n_samples < min_samples) {
        stop("SummarizedExperiment has ", n_samples, " samples (need at least ",
            min_samples, ")")
    }
    ...
    if (!is.null(assay_name)) {
        ...
        if (!assay_name %in% assay_names) {
            stop("Assay '", assay_name, "' not found.\n", "  Available assays: ",
                paste(assay_names, collapse = ", "), "\n", "  Ensure you've run: .calculate_diversity(..., assay_name = '",
                assay_name, "')")
        }
    }
...
#' @noRd
.validate_df <- function(df, required_cols = NULL, allow_null = FALSE, df_name = "Input data") {
    # Check for NULL
    if (is.null(df)) {
        if (allow_null)
            return(invisible(TRUE))
    ...
    if (!is.null(required_cols)) {
        ...
        if (length(missing_cols) > 0) {
            stop("'", paste(missing_cols, collapse = "', '"), "' column(s) missing in ",
...
#' @noRd
.validate_numeric <- function(value, param_name = "parameter", allow_null = FALSE,
    min = NULL, max = NULL, integer_only = FALSE) {
    # Check for NULL
    if (is.null(value)) {
        if (allow_null)
            return(invisible(TRUE))
...
#' @noRd
.validate_choice <- function(value, param_name = "parameter", allowed_values = NULL,
    allow_null = FALSE) {
    # Check for NULL
    if (is.null(value)) {
        if (allow_null)
            return(invisible(TRUE))
        stop(param_name, " is NULL. Choose one of: ", paste(allowed_values, collapse = ", "))
    }

    # Check not in allowed
    if (!value %in% allowed_values) {
        stop(param_name, " must be one of: ", paste(allowed_values, collapse = ", "),
            " (got '", value, "')")
    }

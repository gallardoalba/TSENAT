...
.plot_tsallis_divergence_profile <- function(se, gene = NULL, lm_res = NULL, readcounts = NULL,
    tx2gene_map = NULL, group_col = "group", n_top = 3, assay_name = "diversity",
    arrange_type = c("facet", "list"), signed = TRUE) {
    # ===== INPUT VALIDATION =====

    # Validate SummarizedExperiment
    if (!inherits(se, "SummarizedExperiment")) {
        stop("Argument 'se' must be a SummarizedExperiment object, got: ", class(se)[1])
    }

    # Validate assay_name exists
    if (!is.character(assay_name) || length(assay_name) != 1) {
        stop("Argument 'assay_name' must be a single character string, got: ", class(assay_name)[1])
    }

    if (!assay_name %in% names(SummarizedExperiment::assays(se))) {
        stop("Assay '", assay_name, "' not found in SummarizedExperiment. ", "Available assays: ",
    ...
    if (!is.character(group_col) || length(group_col) != 1) {
        stop("Argument 'group_col' must be a single character string, got: ", class(group_col)[1])
    }

    col_data <- as.data.frame(SummarizedExperiment::colData(se))
    if (!group_col %in% colnames(col_data)) {
        stop("Column '", group_col, "' not found in colData(se). ", "Available columns: ",
    ...
    if (length(groups) != 2) {
        stop("Exactly 2 groups required in '", group_col, "' column; found ", length(groups),
    ...
    plot_data <- .select_genescompute_divergence(se = se, genes = genes, q_values = q_values,
        unique_q = unique_q, groups = groups, group_col = group_col, assay_name = assay_name,
        readcounts = readcounts, tx2gene_map = tx2gene_map, signed = signed)
...
.select_genesselect_genes <- function(gene, lm_res, n_top) {
    ...
    if (!("gene" %in% colnames(lm_res))) {
        stop("'lm_res' must have a 'gene' column. Found columns: ", paste(colnames(lm_res),
            collapse = ", "))
    }
...
.select_genescompute_divergence <- function(se, genes, q_values, unique_q, groups,
    group_col, assay_name, readcounts, tx2gene_map, signed) {
    plot_data_list <- list()

    for (gene_name in genes) {
        divergences <- vapply(unique_q, function(q) {
            .select_genescalc_div_for_gene_q(se = se, gene_name = gene_name, q_val = q,
                q_values = q_values, groups = groups, group_col = group_col, assay_name = assay_name,
                readcounts = readcounts, tx2gene_map = tx2gene_map, signed = signed)
        }, FUN.VALUE = numeric(1))

        df_gene <- data.frame(gene = gene_name, q = unique_q, divergence = divergences,
    ...
    if (nrow(all_plot_data) > 0 && signed) {
        all_plot_data$direction <- ifelse(all_plot_data$divergence > 0, paste0("Positive: ",
            groups[2], " higher"), paste0("Negative: ", groups[1], " higher"))
    }
...
.select_genescalc_div_for_gene_q <- function(se, gene_name, q_val, q_values, groups,
    group_col, assay_name, readcounts, tx2gene_map, signed) {
    ...
    if (!is.null(readcounts) && !is.null(tx2gene_map)) {
        div <- tryCatch({
            calculate_tsallis_divergence_paired_gene(gene_name = gene_name, gene_data = data.frame(entropy = entropy_vals,
                q = q_val, group = group_vals, sample = colnames(se)[cols_q], stringsAsFactors = FALSE),
                readcounts = readcounts, tx2gene_map = tx2gene_map, entropy_pred = data.frame(entropy_pred = entropy_vals,
                  q = q_val, group = group_vals, stringsAsFactors = FALSE), group_levels = groups)
        }, error = function(e) {
            NA_real_
        })
    ...
    if (signed) {
        p <- ggplot2::ggplot(plot_data, ggplot2::aes(x = q, y = divergence, color = direction)) +
            ggplot2::geom_hline(yintercept = 0, linetype = "dashed", color = "gray50",
                linewidth = 0.8) + ggplot2::geom_line(linewidth = 1.1) + ggplot2::geom_point(size = 3,
            alpha = 0.7) + ggplot2::scale_color_manual(name = "Divergence Direction:",
            values = setNames(c("#E63946", "#1D3557"), c(paste0("Negative: ", groups[1],
                " higher"), paste0("Positive: ", groups[2], " higher")))) + ggplot2::labs(title = "Tsallis Divergence Profile: Directional (Signed)",
            x = "q value (diversity scale parameter)", y = "Divergence D[q]") + .theme_spectrum(base_size = 11) +
        ...
        p <- ggplot2::ggplot(plot_data, ggplot2::aes(x = q, y = divergence, color = gene)) +
            ggplot2::geom_line(linewidth = 1.1) + ggplot2::geom_point(size = 3, alpha = 0.7) +
            ggplot2::labs(title = "Tsallis Divergence Profile Across q-Spectrum",
                x = "q value (diversity scale parameter)", y = "Divergence D[q] (Absolute)",
                color = "Gene") + .theme_spectrum(base_size = 11) + ggplot2::theme(plot.title = ggplot2::element_text(size = .font_sizes$title),
            axis.title = ggplot2::element_text(size = .font_sizes$axis_title), axis.text = ggplot2::element_text(size = .font_sizes$axis_text))
    }

    # Add faceting if multiple genes
    if (length(unique(plot_data$gene)) > 1) {
        p <- p + ggplot2::facet_wrap(~gene, scales = "free_y") + ggplot2::theme(legend.position = "top")
    }
    ...
    for (gene_name in genes) {
        ...
        if (signed) {
            p_gene <- ggplot2::ggplot(df_gene, ggplot2::aes(x = q, y = divergence,
                fill = direction, color = direction)) + ggplot2::geom_hline(yintercept = 0,
                linetype = "dashed", color = "gray50", linewidth = 0.8) + ggplot2::geom_line(linewidth = 1.2) +
                ggplot2::geom_point(size = 3.5, alpha = 0.8) + ggplot2::scale_color_manual(name = "Divergence Direction:",
                values = setNames(c("#E63946", "#1D3557"), c(paste0("Negative: ",
                  groups[1], " higher"), paste0("Positive: ", groups[2], " higher")))) +
                ggplot2::scale_fill_manual(name = "Divergence Direction:", values = setNames(c("#E63946",
                  "#1D3557"), c(paste0("Negative: ", groups[1], " higher"), paste0("Positive: ",
                  groups[2], " higher")))) + ggplot2::labs(title = paste("Divergence Profile (Signed):",
                gene_name), x = "q value", y = expression("Divergence D[q]"), subtitle = paste0("Red = ",
                groups[1], " higher | Blue = ", groups[2], " higher")) + .theme_spectrum(base_size = 11) +
                ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5, size = .font_sizes$title,
                  face = "bold"), panel.grid.minor = ggplot2::element_blank(), legend.position = "right")
        } else {
            p_gene <- ggplot2::ggplot(df_gene, ggplot2::aes(x = q, y = divergence)) +
                ggplot2::geom_line(color = "#2E86AB", linewidth = 1.2) + ggplot2::geom_point(color = "#2E86AB",
                size = 3.5, alpha = 0.8) + ggplot2::labs(title = paste("Divergence Profile:",
                gene_name), x = "q value", y = "Divergence D[q] (Absolute)") + .theme_spectrum(base_size = 11) +
                ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5, size = .font_sizes$title,
                  face = "bold"), panel.grid.minor = ggplot2::element_blank())
        }

...
.detect_salmon_samples <- function(
  ...
  if (!dir.exists(salmon_dir)) {
    stop(
      "[.detect_salmon_samples] Directory does not exist: ", salmon_dir, "\n",
  ...
  if (length(file_paths) == 0) {
    stop(
      "[.detect_salmon_samples] No Salmon quantification files found in: ", salmon_dir, "\n",
  ...
  # Extract sample names from parent directory paths
  # For: /path/to/salmon/sample1/quant.sf -> "sample1"
  ...
  if (length(unique(sample_names)) != length(sample_names)) {
    warning(
      "[.detect_salmon_samples] Found duplicate sample names. ",
      "This may indicate nested sample folders with same names."
    )
  }
...
.validate_salmon_files <- function(file_paths, verbose = TRUE) {
  ...
  if (length(missing_files) > 0) {
    stop(
      "[.validate_salmon_files] File(s) not found:\n",
      paste("  -", file_paths[missing_files], collapse = "\n")
    )
  }
  ...
  if (verbose) message("[.validate_salmon_files] Checking format of: ", basename(first_file))

  tryCatch(
    {
      first_data <- readr::read_tsv(
        first_file,
      ...
      if (length(missing_cols) > 0) {
        stop(
          "[.validate_salmon_files] Missing required columns: ",
    ...
    error = function(e) {
      stop(
        "[.validate_salmon_files] Error reading first file: ", basename(first_file), "\n",
  ...
  if (length(file_paths) > 1) {
    if (verbose) {
      message("[.validate_salmon_files] Checking consistency across ", length(file_paths), " files...")
    }
      ...
      tryCatch(
        {
          ...
          if (!identical(transcript_ids_first, transcript_ids_current)) {
            ...
            warning(
              "[.validate_salmon_files] Transcript ID mismatch between files:\n",
              warnings_msg,
              "  Suggestion: Ensure all files use same transcriptome reference"
            )
          }

          if (nrow(current_data) == 0) {
            stop("[.validate_salmon_files] File is empty (no data rows): ", basename(current_file))
          }
        },
        error = function(e) {
          stop(
            "[.validate_salmon_files] Error reading file ", i, ": ", basename(current_file), "\n",
            "  Details: ", conditionMessage(e)
          )
        }
      )
...
.read_salmon_samples <- function(
  ...
  if (length(sample_names) != length(file_paths)) {
    stop(
      "[.read_salmon_samples] Length mismatch: ",
      length(file_paths), " files but ", length(sample_names), " sample names"
    )
  }

  # Validate files before processing
  if (verbose) message("[.read_salmon_samples] Validating ", length(file_paths), " files...")
  ...
  if (verbose) message("[.read_salmon_samples] Reading transcript IDs from: ", basename(file_paths[1]))
  ...
  if (verbose) {
    message(
      "[.read_salmon_samples] Found ", n_transcripts, " transcripts ",
  ...
  if (include_tpm) tpm_matrix[, 1] <- first_data$TPM
  if (include_eff_length) eff_length_matrix[, 1] <- first_data$EffectiveLength

...
.calculate_rank_assumptions <- function(data, checks = "rank", alpha = 0.05, q_values = NULL, gee_params = list()) {
    ...
    if (is.character(checks) && length(checks) == 1) {
        if (checks == "rank") {
            ...
            checks <- c("exchangeability", "monotonicity", "consistency", "gam_metrics", "gee_metrics", "lmm_metrics", "fpca_metrics")
        } else {
            # Allow single metric names to pass through
            # They will be used as-is in the checks below
        }
    ...
    if ("exchangeability" %in% checks) {
        ...
        if (ncol(data) > 2) {
            ...
            # Interpretation: if p > alpha, data is exchangeable (no ordering effect)
            exchangeability_interpretation <- if (p_exchangeability > alpha) "exchangeable" else "ordering detected"
            ...
            results$exchangeability <- list(description = "Sample exchangeability (serial correlation in sequence)",
                ...
                status = exchangeability_interpretation,
...
print.rank_assumptions <- function(x, ...) {
    ...
    if (!is.null(check_results)) {
        ...
        rank_checks <- setdiff(names(check_results), c("gam_metrics", "gee_metrics", "lmm_metrics"))
            ...
            for (metric_name in names(gee_results)) {
                ...
                if (metric_name == "correlation_fit") {
                    ...
                    if (!is.na(metric$mean_residual_correlation)) {
                        message(sprintf("  Mean within-cluster correlation: %.3f", 
                                      metric$mean_residual_correlation))
                    }
            ...
            for (metric_name in names(lmm_results)) {
                ...
                if (metric_name == "variance_components") {
                    ...
                    if (!is.null(metric$shapiro_pvalue) && !is.na(metric$shapiro_pvalue)) {
                        ...
                        message(sprintf("  Skewness: %.3f, Kurtosis: %.3f", 
                                      metric$skewness, metric$kurtosis))
                    }
                    ...
                    if (!is.null(metric$n_outliers)) {
                        ...
                        message(sprintf("  Influential points: %.1f%%", metric$prop_influential * 100))
                    }
                }
            ...
            for (metric_name in names(fpca_results)) {
                ...
                if (metric_name == "variance_adequacy") {
                    ...
                    if (!is.null(metric$cv_mean) && !is.na(metric$cv_mean)) {
                        ...
                        message(sprintf("  Stable CIs (excl. zero): %.0f%%", metric$prop_stable_cis * 100))
                    }
                }
...
.compute_working_correlation_fit <- function(data, assumed_structure = "exchangeable") {
    ...
    tryCatch({
        ...
        return(list(
            description = "Working Correlation Structure Fit",
            assumed_structure = assumed_structure,
            mean_autocorrelation = mean_autocorr,
            suitable_structures = suitable_structures,
            status = fit_status,
            details = sprintf("Observed autocorr=%.3f; %s", mean_autocorr, assessment)
        ))
...
.compute_cluster_size_variation <- function(data, cluster_col = NULL) {
    ...
    tryCatch({
        ...
        cv_size <- if (is.na(sd_size)) 0 else sd_size / mean_size  # Coefficient of variation (0 if only 1 cluster)
        ...
        return(list(
            description = "Cluster Size Variation",
            n_clusters = length(cluster_sizes),
            mean_cluster_size = mean_size,
            sd_cluster_size = sd_size,
            cv_cluster_size = cv_size,
            min_size = min_size,
            max_size = max_size,
            status = status,
            details = sprintf("Mean size=%.1f (CV=%.3f, range %d-%d) - %s", 
...
.compute_independence_residuals <- function(data, cluster_col = NULL) {
    ...
    tryCatch({
        ...
        return(list(
            description = "Within-Cluster Independence Test",
            mean_residual_correlation = mean_within_cor,
            n_clusters_tested = sum(!is.na(within_cluster_cors)),
            status = status,
...
.compute_gee_scale_parameter <- function(data, cluster_col = NULL) {
    ...
    tryCatch({
        ...
        # Estimate scale parameter as variance ratio
        # Assuming Gaussian family: phi = observed_var / expected_var
        # For standardized comparison, use normalized variance
        if (mean_val > 0) {
            scale_param <- var_val / (mean_val^2)  # Roughly variance/mean ratio
        } else {
            scale_param <- if (abs(mean_val) > 0) var_val / abs(mean_val) else var_val
        }
...
.get_gee_metrics <- function(data, gee_params = list()) {
    ...
    results <- list(
        correlation_fit = .compute_working_correlation_fit(data, assumed_structure = assumed_structure),
    ...
    results$consolidated <- list(
        description = "Generalized Estimating Equations diagnostics",
        result = consolidated_result,
        status = "COMBINED"
    )
...
.compute_variance_components <- function(data, cluster_col = NULL) {
    ...
    if (ncol(data) < 2) {
        return(list(
            description = "Variance Components (Random Effects vs Residual)",
            status = "? SKIP",
            details = "Need at least 2 columns for LMM variance decomposition"
        ))
    }
    
    tryCatch({
        # Simple variance decomposition without requiring lme4
        # Treat columns as repeated measures or clusters
        ...
        icc <- if (!is.na(between_var) && !is.na(within_var) && (between_var + within_var) > 0) {
...
.compute_random_effects_normality <- function(data, cluster_col = NULL) {
    ...
    tryCatch({
        ...
        if (length(col_means) < 3) {
            return(list(
                description = "Random Effects Normality",
                status = "? SKIP",
...
.compute_variance_homogeneity <- function(data, cluster_col = NULL) {
    ...
    tryCatch({
        ...
        group_variances <- apply(data, 2, function(x) {
            x_clean <- x[!is.na(x)]
            if (length(x_clean) > 1) stats::var(x_clean) else NA_real_
        })
        ...
        if (length(group_variances) < 2) {
            return(list(
                description = "Variance Homogeneity (Levene's Test)",
                status = "? SKIP",
        ...
        levene_statistic <- if (!is.na(ms_between) && !is.na(ms_within) && ms_within > 0) {
        ...
        return(list(
            description = "Variance Homogeneity",
            status = "? ERROR",
            details = paste("Failed:", e$message)
        ))
    })
...
.compute_lmm_influence <- function(data, cluster_col = NULL) {
    ...
    tryCatch({
        ...
        if (length(data_clean) < 3) {
            return(list(
                description = "Outlier Influence Assessment",
                status = "? SKIP",
        ...
        # Cook's distance approximation: (std_residual)^2
        # For exploratory use without actual regression model
        # Threshold = 1 is a standard cutoff for Cook's distance
        # (Note: 4/n would be too small for large samples, flagging nearly all points)
        ...
        return(list(
            ...
            prop_influential = prop_influential,
            n_observations = length(data_clean),
            status = status,
            details = sprintf("Outliers=%d, Extreme=%d, Influential=%.1f%%", 
...
.get_lmm_metrics <- function(data, lmm_params = list()) {
    ...
    # Compute each metric independently
    results <- list(
        variance_components = .compute_variance_components(data, cluster_col = cluster_col),
    ...
    results$consolidated <- list(
        description = "Linear Mixed Models diagnostics",
        result = consolidated_result,
        status = "COMBINED"
    )
...
.compute_cumulative_variance_fpca <- function(data, max_components = NULL) {
    ...
    tryCatch({
        # Compute PCA via singular value decomposition (functional PCA approximation)
        # For functional data, we treat rows as observations and columns as function values
        ...
        if (length(eigenvalues) == 0) {
            return(list(
                description = "Functional PCA Dimension Adequacy",
                status = "? SKIP",
        ...
        if (length(n_comp_90) == 0) n_comp_90 <- length(eigenvalues)
        if (length(n_comp_95) == 0) n_comp_95 <- length(eigenvalues)
        if (length(n_comp_99) == 0) n_comp_99 <- length(eigenvalues)
        ...
        return(list(
            ...
            n_components_99 = n_comp_99,
            total_variance = total_variance,
            n_components = length(eigenvalues),
            status = status,
            details = sprintf(
                "Components for 90%%=%.0f, 95%%=%.0f, 99%%=%.0f. %s",
...
.compute_fpca_bootstrap_stability <- function(data, n_bootstrap = 500, n_components = 3) {
    ...
    tryCatch({
        ...
        return(list(
            ...
            cv_eigenvalues = cv_eigenvalues,
            cv_mean = cv_mean,
            max_cv = max_cv,
            prop_stable_cis = prop_stable,
            status = status,
            details = sprintf(
                "Bootstrap SE (mean CV)=%.3f; Stable CIs=%.0f%%. %s",
...
.get_fpca_metrics <- function(data, fpca_params = list()) {
    ...
    # Consolidated result
    status_va <- if (is.null(variance_adequacy$status)) "?" else 
        sub("^\\? ", "", variance_adequacy$status)
    status_bs <- if (is.null(bootstrap_stability$status)) "?" else 
        sub("^\\? ", "", bootstrap_stability$status)
    
    consolidated_result <- paste(
        "Variance:", status_va, "|",
        "Bootstrap:", status_bs
    )
    
    results <- list(
        variance_adequacy = variance_adequacy,
        bootstrap_stability = bootstrap_stability,
        consolidated = list(
            description = "Functional Principal Component Analysis diagnostics",
            result = consolidated_result,
            status = "COMBINED"
        )
    )

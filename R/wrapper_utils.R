...
resolve_slot_param <- function(user_value, config_list, config_key, 
  ...
  # No value found
  if (!allow_null) {
    param_desc <- if (is.null(description)) config_key else description
    stop(
      "Could not resolve parameter '", param_desc, "'. ",
      "Please provide either: user argument, config['", config_key, "'], or update allow_null=TRUE",
      call. = FALSE
    )
  }
...
auto_detect_column <- function(available_cols, config_list = NULL, config_key = NULL,
  ...
  if (length(priority_candidates) > 0) {
    ...
    if (!is.na(first_match_idx) && first_match_idx <= length(idx) && !is.na(idx[first_match_idx])) {
      ...
      if (verbose && !is.null(param_name)) {
        message(sprintf("[auto_detect_column] Auto-detected %s = %s", param_name, detected))
      }
  ...
  if (!is.null(default_fallback)) {
    if (verbose && !is.null(param_name)) {
      message(sprintf("[auto_detect_column] Using fallback for %s = %s", param_name, default_fallback))
    }
...
save_analysis_output <- function(data, output_file, object = NULL, verbose = FALSE,
  ...
  if (create_dir) {
    ...
    if (output_dir != "." && !dir.exists(output_dir)) {
      tryCatch({
        ...
        warning(
          "[", func_name, "] Could not create output directory: ",
          output_dir, " (", conditionMessage(e), ")",
          call. = FALSE
        )
      })
    }
  ...
  tryCatch({
    if (ext %in% c(".tsv", ".csv", ".txt")) {
      ...
      sep <- if (ext == ".tsv") "\t" else ifelse(ext == ".csv", ",", "\t")
      ...
      if (!inherits(data, "ggplot")) {
        warning(
          "[", func_name, "] Cannot save non-ggplot object as ", ext,
          ". Skipping plot output.",
          call. = FALSE
        )
        return(invisible(FALSE))
      }
      ...
      # Unknown format
      warning(
        "[", func_name, "] Unknown output format: ", ext, 
        ". Trying default RDS save.",
        call. = FALSE
      )
      saveRDS(data, file = output_file)
    }
    ...
    warning(
      "[", func_name, "] Could not save to ", output_file, ": ",
      conditionMessage(e),
      call. = FALSE
    )
    return(invisible(FALSE))
...
extract_multiq_table <- function(result, is_multiq = NULL, extract_fn = NULL,
  ...
  if (is.null(is_multiq)) {
    is_multiq <- inherits(result, "tsenat_isoform_switching_multiq") ||
                 (is.list(result) && any(grepl("^q_", names(result))))
  }

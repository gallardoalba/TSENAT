...
# ============================================================================
# HELPER: Get gene IDs from rowData or tx2gene mapping
# Returns NULL if no gene mapping available
# @noRd
.get_gene_ids <- function(se) {
    ...
    if (!is.null(md$tx2gene) && is.data.frame(md$tx2gene) && nrow(md$tx2gene) > 0) {
        ...
        if (ncol(tx2gene) >= 2) {
            ...
            # Match rownames to transcript column and return gene IDs in correct order
...
# ============================================================================
# HELPER: Validate filter parameters for type and range
...
.validate_filter_params <- function(min_isoform_abundance, tpm_assay_name) {
    if (!is.null(min_isoform_abundance)) {
        if (!is.numeric(min_isoform_abundance) || length(min_isoform_abundance) != 1) {
...
# Implements Law et al. limma-voom methodology
# @noRd
...
# ============================================================================
# HELPER: Calculate min_samples and min_tx_per_gene based on stringency
# @noRd
.calculate_stringency_thresholds <- function(stringency, n_samples, n_pairs = NULL) {
    ...
    list(min_samples = min_samples, min_tx_per_gene = min_tx_per_gene, 
         min_isoform_abundance = min_isoform_abundance)
}

# ============================================================================
# HELPER: Detect pair column from colData
# Tries multiple candidate names for pairing information
# @noRd
.detect_pair_column <- function(col_data, se) {
    # Candidate column names to try (in order of priority)
    candidates <- c("pair", "pair_id", "pair_num", "pair_number", 
                    "subject", "subject_id", "replicate_id", "rep_id")
...
# ============================================================================
# HELPER: Calculate stringency parameters from SummarizedExperiment
# Wrapper around .calculate_stringency_thresholds that works with SE objects
# @noRd
...
# Returns logical vector indicating which rows to keep
# @noRd
.apply_tpm_filter <- function(assay_mat, min_tpm, min_samples, verbose = FALSE) {
    ...
    if (verbose) {
        message(sprintf("TPM-based filtering: min_tpm = %.3f in >= %d samples",
                       min_tpm, eff_min_samples))
    }
...
# ============================================================================
# HELPER: Filter genes by minimum transcripts per gene
# Returns updated logical vector
# @noRd
...
# ============================================================================
# HELPER: Filter isoforms by relative abundance within genes
# Implements Soneson et al. (2016) isoform-level filtering
# Returns updated logical vector
# @noRd
.filter_by_isoform_abundance <- function(tokeep, genes_vec, assay_mat, 
                                        min_isoform_abundance, verbose = FALSE) {
    if (is.null(min_isoform_abundance) || min_isoform_abundance <= 0 || 
        is.null(genes_vec) || length(genes_vec) == 0) {
        return(tokeep)
    }
...
# ============================================================================
# HELPER: Filter genes by valid sample fraction
# Removes genes sparse in too many samples
# Returns updated logical vector
# @noRd


# ============================================================================
# HELPER: Synchronize metadata after filtering
# Updates readcounts, tx2gene, salmon_tpm, etc. in metadata
# Returns updated metadata list
# @noRd
.sync_filter_metadata <- function(md, tokeep, assay_mat, rownames_se = NULL) {
    # Get the rows that are being kept
    # If rownames_se not provided, try to extract from metadata
    ...
    if (!is.null(md$salmon_effective_length)) {
        if (is.vector(md$salmon_effective_length)) {
            ...
            } else if (is.numeric(md$salmon_effective_length) && length(md$salmon_effective_length) == length(rownames_se)) {
                # If unnamed vector same length as original rows, subset by index
...
# ============================================================================
# HELPER: Construct filtered SummarizedExperiment
# Returns new SE object with filtered data
# @noRd
...
.filter_se <- function(se, min_samples = 5L, stringency = NULL,
    ...
    if (!is.null(min_isoform_abundance)) {
        if (!is.numeric(min_isoform_abundance) || length(min_isoform_abundance) != 1) {
            stop(".filter_se: 'min_isoform_abundance' must be numeric in [0, 1], or NULL to use defaults", call. = FALSE)
        }
        if (min_isoform_abundance < 0 || min_isoform_abundance > 1) {
            stop(".filter_se: 'min_isoform_abundance' must be numeric in [0, 1], or NULL to use defaults", call. = FALSE)
        }
    ...
    if (!is.null(stringency)) {
        ...
        stringency_result <- .calculate_stringency_thresholds(stringency, n_samples, n_pairs)
        min_samples <- stringency_result$min_samples
        min_tx_per_gene <- stringency_result$min_tx_per_gene
        
        # Use stringency-based min_isoform_abundance, but allow explicit override
        if (is.null(min_isoform_abundance)) {
        ...
        # Use helper to estimate min_tpm (returns list with min_tpm and quant_label)
    ...
    # If min_isoform_abundance not specified and no stringency, use default 0.05
    if (is.null(min_isoform_abundance)) {
        min_isoform_abundance <- 0.05
    }

    # Get assay matrix for filtering (OPTIMIZATION: reuse assays_list from top; use TPM data if found, otherwise use specified assay)
    if (!is.null(tpm_assay_mat)) {
    ...
    tokeep <- .filter_by_isoform_abundance(tokeep, genes_vec, assay_mat, 
                                          min_isoform_abundance, verbose = verbose)
...
.detect_paired_structure <- function(coldata) {
  ...
  for (col in pair_cols) {
    if (col %in% colnames(coldata)) {
      # Verify it's a valid pairing: check if ANY pairs appear in each condition
      if ("condition" %in% colnames(coldata)) {
        ...
        # A paired design exists if there are pairs that appear in multiple conditions
        # Find "complete pairs" - those that appear in all conditions
        complete_pairs <- rownames(pair_condition_table)[
          rowSums(pair_condition_table > 0) == length(conditions)
        ]
...
.select_pairs_balanced <- function(coldata, n_samples, pair_col, seed, verbose = FALSE) {
  ...
  if (n_samples %% 2 != 0) {
    ...
    if (verbose) {
      message("[subset_analysis] Rounding up to ", n_samples, 
              " samples for complete pair selection")
    }
  }
...
.balance_sample_selection <- function(coldata, n_samples, seed, verbose = FALSE) {
  ...
  # BUG FIX (April 2026): Preserve paired structure when subsampling
  # Detect if this is a paired design and handle accordingly
  ...
  if (pairing_info$is_paired && length(pairing_info$complete_pairs) > 0) {
    ...
    pair_samples <- .select_pairs_balanced(
      coldata = coldata,
      n_samples = n_samples,
      pair_col = pairing_info$pair_col,
      seed = seed,
      verbose = verbose
    )
  ...
  if (!is.null(samples)) {
    ...
    if (verbose) {
      n_conditions <- if ("condition" %in% colnames(coldata)) 
        length(unique(coldata$condition)) 
      else if ("sample_type" %in% colnames(coldata))
        length(unique(coldata$sample_type))
      else
        1
      
      if (n_conditions > 1) {
        message("[subset_analysis] Selected ", n_samples, 
                " samples balanced across ", n_conditions, " conditions")
      } else {
...
.subset_analysis <- function(
  ...
  gene_idx <- .select_genes_from_analysis(
    se = se,
    n_genes = n_genes,
    genes = genes,
    select_by = select_by,
  ...
  sample_idx <- .select_samples_from_analysis(
    se = se,
    n_samples = n_samples,
    samples = samples,
    seed = seed,
  ...
  analysis_subset <- .sync_subset_metadata(
    analysis_obj = analysis_subset,
    gene_idx = gene_idx,

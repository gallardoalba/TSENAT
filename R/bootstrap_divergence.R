...
.bootstrap_divergence <- function(x = NULL, y = NULL, se = NULL, res = NULL, top_n = 1,
    ...
    data <- .bootstrap_divergence_extract_data(x, y, se, res, top_n, group_col, 
        control_group)
    x <- data$x
    y <- data$y
    if (!is.null(data$gene_name) && is.null(gene_name)) gene_name <- data$gene_name
    ...
    bootstrap_divs <- .bootstrap_divergence_compute_samples(x, y, nboot, q, 
        pseudocount, log_base, paired, se, pair_id_col, group_col, control_group)
    ...
    ci_result <- .bootstrap_divergence_compute_ci(valid_divs, estimate, method, 
        nboot, ci)
    
    # Create result object
    result <- list(estimate = estimate, lower_ci = ci_result$lower, 
...
#' @noRd
.bootstrap_divergence_extract_data <- function(x, y, se, res, top_n, group_col, 
    control_group) {
    ...
    if (verbose && !is.null(gene_name)) {
        ...
        for (qi_idx in seq_along(q)) {
            ...
            message(sprintf("q = %.2f: D_q = %.4f [%.4f, %.4f] (width=%.4f)",
                q[qi_idx], r$estimate, r$lower_ci, r$upper_ci, r$upper_ci - r$lower_ci))
        }
        message("")
    }
...
#' @noRd
.bootstrap_divergence_compute_ci <- function(valid_divs, estimate, method, 
    nboot, ci) {
    alpha <- 1 - ci
...
#' @noRd
.bootstrap_divergence_print_single <- function(result, gene_name, q, nboot, 
    method, ci) {
    message("")
...
#' @noRd
.bootstrap_divergence_compute_samples <- function(x, y, nboot, q, pseudocount, 
    log_base, paired, se, pair_id_col, group_col, control_group) {
    if (isTRUE(paired) && !is.null(se)) {
        pair_ids_numeric <- .bootstrap_divergence_compute_paired_samples(se, 
            pair_id_col, group_col, control_group, x, y)
        bootstrap_divs <- divergence_bootstrap_paired_cpp_wrapper(x = as.numeric(x),
            ...
            q = as.numeric(q), pseudocount = as.numeric(pseudocount), 
            log_base = as.numeric(log_base))
    } else {
        bootstrap_divs <- divergence_bootstrap_compute_cpp_wrapper(x = as.numeric(x),
            y = as.numeric(y), nboot = as.integer(nboot), q = as.numeric(q), 
            pseudocount = as.numeric(pseudocount), log_base = as.numeric(log_base), 
            paired = FALSE)
    }
    bootstrap_divs
}

...
extract_tx2gene_from_gff3 <- function(gff3_file) {
    ...
    # Pre-allocate vectors for efficiency (start with capacity for 10k transcripts)
    # This avoids repeatedly growing lists/data.frames which is slow
    ...
    while (TRUE) {
        ...
        for (line in lines) {
            ...
            # Parse GFF3 line format: seqname source feature start end score strand phase attributes
            # Use strsplit only once per line
            fields <- strsplit(line, "\t", fixed = TRUE)[[1]]
            if (length(fields) < 9) next
            
            # Extract feature type (3rd column) - do this check first to skip early
            feature_type <- fields[3]
            if (!feature_type %in% c("transcript", "mRNA")) next
            
            # Only now extract attributes from the (potentially long) 9th column
            attributes <- fields[9]
            
            # Extract ID and Parent using efficient substring operations
            # Find positions of ID= and Parent= patterns
            ...
            if (id_start > 3) {
                ...
                if (id_end > 0) {
                    transcript_id <- substr(attributes, id_start, id_start + id_end - 2)
                } else {
            ...
            if (parent_start > 7) {
                ...
                if (parent_end > 0) {
                    gene_id <- substr(attributes, parent_start, parent_start + parent_end - 2)
                } else {
    ...
    if (idx == 0) {
        stop("No transcript-gene mappings found in GFF3 file. ", 
             "Ensure the file contains 'transcript' or 'mRNA' features with ID and Parent attributes.", 
             call. = FALSE)
    }
    
    # Trim to actual size and create data frame
    tx2gene_df <- data.frame(
        Transcript = tx2gene_transcripts[1:idx],
        Gene = tx2gene_genes[1:idx],

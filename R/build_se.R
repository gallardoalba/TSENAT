...
build_se <- function(tx2gene_tsv, readcounts, genes, assay_name = "counts") {
    ...
    if (!is.matrix(readcounts) || !is.numeric(readcounts)) stop("'readcounts' must be a numeric matrix or numeric data.frame.", call. = FALSE)

    if (length(genes) != nrow(readcounts)) stop("Length of 'genes' must equal nrow(readcounts).", call. = FALSE)
    ...
    if (is.null(rn)) {
        ...
        SummarizedExperiment::rowData(se) <- S4Vectors::DataFrame(genes = genes, row.names = rn)
    }

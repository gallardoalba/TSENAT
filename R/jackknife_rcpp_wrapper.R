# Rcpp wrapper and fallback infrastructure for jackknife resampling
# 
# Purpose: Accelerate jackknife computation with optional C++ implementation
# Fallback: Pure R version available if Rcpp unavailable
...
.initialize_rcpp_check <- function() {
  ...
  if (!cached) {
    ...
    options(
      tsenat.rcpp_initialized = TRUE,
      tsenat.rcpp_is_available = rcpp_available
    )
...
#' @keywords internal
#' @export
# Internal wrapper function for C++ jackknife resampling
# (exported in NAMESPACE but no man page)
...
.jackknife_resampling_hybrid <- function(counts, entropy_fn = .entropy_core, 
  ...
  result <- list(
    estimate = result_cpp$estimate,
    jackknife_estimates = result_cpp$jackknife_estimates,
    influence = result_cpp$influence,
    jackknife_se = result_cpp$jackknife_se,
    outlier_indices = outlier_indices,
    outlier_cutoff_value = unname(outlier_cutoff),
    outlier_threshold = threshold,
    n_observations = nrow(counts)
  )
...
# Set the hybridversion as the primary implementation
# Note: R implementation has been removed and moved to C++
...
viz_rcpp_status <- function() {
  list(
    rcpp_available = .initialize_rcpp_check(),
    description = "Rcpp acceleration for jackknife resampling computation"
  )
}

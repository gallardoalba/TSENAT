...
.rank_correlation_bootstrap_ci <- function(pvalues_or_ranks, method = c("spearman",
    "kendall"), ci = c("percentile", "bca", "permutation"), ci_level = 0.95, n_bootstrap = "auto",
    n_permutations = 5000, seed = 42, return_distribution = FALSE, nthreads = 1) {
    ...
    q_names <- if (is.null(names(rank_list)))
        paste0("q", seq_len(n_q)) else names(rank_list)

    # OPTIMIZATION: Vectorize correlation matrix computation Convert rank_list
    # to matrix and compute correlation in single call instead of n_q²
    # individual cor() calls in nested loops
    ...
    if (ci == "percentile" || ci == "bca") {
        ...
        # PARALLELIZED: Use mclapply to compute bootstrap correlations in
        # parallel Each bootstrap sample is independent, perfect for
        # parallelization
        bootstrap_results <- parallel::mclapply(X = seq_len(n_bootstrap), FUN = function(b) {
        ...
        if (ci == "percentile") {
            ...
            ci_matrix <- array(NA, dim = c(n_q, n_q, 2), dimnames = list(q_names,
                q_names, c("lower", "upper")))

            # VECTORIZED: Use apply() to compute quantiles for all pairs at
            # once This replaces nested loops with vectorized array operations
            lower_quantiles <- apply(bootstrap_corrs, c(1, 2), quantile, probs = alpha/2,
                na.rm = TRUE)
            upper_quantiles <- apply(bootstrap_corrs, c(1, 2), quantile, probs = 1 -
                alpha/2, na.rm = TRUE)
            ...
            # BCA method Calculate bias-correction and acceleration factors
            ci_matrix <- array(NA, dim = c(n_q, n_q, 2), dimnames = list(q_names,
                q_names, c("lower", "upper")))
            ...
            for (i in seq_len(n_q)) {
                for (j in seq_len(n_q)) {
                  ...
                  # Acceleration (jackknife-based) VECTORIZED: Use vapply to
                  # compute jackknife correlations efficiently Pre-compute all
                  # leave-one-out samples at once using matrix indexing
                  jack_corrs <- vapply(seq_len(n_features), function(k) {
                    ...
                    stats::cor(jack_ranks_matrix[, i], jack_ranks_matrix[, j], method = method,
                      use = "complete.obs")
                  }, FUN.VALUE = numeric(1))
                  ...
                  # Bug #2 Fix: Check for near-zero denominator (uniform
                  # jackknife values)
                  if (abs(denominator) < 1e-10) {
                    # Fallback to percentile CI when jackknife correlations are
                    # uniform
                    alpha <- 1 - ci_level
                    ci_matrix[i, j, "lower"] <- quantile(boot_dist, alpha/2, na.rm = TRUE)
                    ci_matrix[i, j, "upper"] <- quantile(boot_dist, 1 - alpha/2,
                      na.rm = TRUE)
                    warning(sprintf("BCA acceleration denominator near zero for Q pair (%s, %s). Falling back to percentile method.",
                      q_names[i], q_names[j]))
                  } else {
                    ...
                    p_lower <- stats::pnorm(z0 + (z0 + z_alpha_lower)/(1 - accel *
                      (z0 + z_alpha_lower)))
                    p_upper <- stats::pnorm(z0 + (z0 + z_alpha_upper)/(1 - accel *
                      (z0 + z_alpha_upper)))
        ...
        bootstrap_dist <- if (return_distribution)
            bootstrap_corrs else NULL

    } else {
        # ci == 'permutation' Exact permutation distribution (PARALLELIZED)
        perm_corrs <- array(NA, dim = c(n_q, n_q, n_permutations))

        # PARALLELIZED: Use mclapply to compute permutation correlations in
        # parallel Each permutation sample is independent, perfect for
        # parallelization
        perm_results <- parallel::mclapply(X = seq_len(n_permutations), FUN = function(p) {
        ...
        alpha <- 1 - ci_level
        ci_matrix <- array(NA, dim = c(n_q, n_q, 2), dimnames = list(q_names, q_names,
            c("lower", "upper")))

        # VECTORIZED: Use apply() to compute quantiles for all permutation
        # pairs
        lower_quantiles <- apply(perm_corrs, c(1, 2), quantile, probs = alpha/2,
            na.rm = TRUE)
        upper_quantiles <- apply(perm_corrs, c(1, 2), quantile, probs = 1 - alpha/2,
            na.rm = TRUE)
        ...
        bootstrap_dist <- if (return_distribution)
            perm_corrs else NULL
    }

    # Interpretation table
    interpretation <- data.frame(Q_value_Pair = character(n_q * (n_q - 1)/2), Correlation = numeric(n_q *
        (n_q - 1)/2), CI_Lower = numeric(n_q * (n_q - 1)/2), CI_Upper = numeric(n_q *
        (n_q - 1)/2), Width = numeric(n_q * (n_q - 1)/2), Includes_Zero = logical(n_q *
        (n_q - 1)/2), Stability = character(n_q * (n_q - 1)/2), stringsAsFactors = FALSE)
    ...
    for (i in seq_len(n_q - 1)) {
        for (j in (i + 1):n_q) {
            ...
            interpretation$Width[idx] <- ci_matrix[i, j, "upper"] - ci_matrix[i,
                j, "lower"]
            ...
            } else if (corr_matrix[i, j] > 0.7) {
                interpretation$Stability[idx] <- "Robust (r > 0.70)"
            } else if (corr_matrix[i, j] > 0.5) {
    ...
    structure(list(correlation_matrix = corr_matrix, ci_matrix = ci_matrix, interpretation = interpretation,
        method = paste(toupper(method), "rank correlation with", ci, "CI"), ci_level = ci_level,
        ci_type = ci, bootstrap_distribution = bootstrap_dist), class = "rank_correlation_ci")
}
...
# Equivalent improvements to LM interaction tests (heteroscedasticity, bounded
# support) APPROACH: 1. Pre-test detection of data characteristics 2.
# Conditional test selection based on characteristics 3. Alternative test
# methods with better properties for detected conditions IMPROVEMENTS: *
# Heteroscedasticity detection -> Aligned Rank Transform (ART) instead of K-W *
# Boundary clustering detection -> Quantile-based comparison * Extreme skewness
# detection -> Robust median test * Default -> Standard Kruskal-Wallis (already
# robust)
# ================================================================================
...
.select_rank_test <- function(data, value_col = "entropy", group_col = "q", verbose = FALSE) {
    ...
    characteristics <- list(heteroscedastic = FALSE, boundary_clustered = FALSE,
        highly_skewed = FALSE, n_groups = nlevels(groups), n_values = length(values))
    ...
    if (!inherits(lin_mod, "try-error")) {
        ...
        if (!inherits(bp_mod, "try-error")) {
            ...
            # Improved: Extract p-value safely (handle various return
            # structures)
            ...
            if (!inherits(bp_anova, "try-error") && nrow(bp_anova) >= 2) {
                ...
                if (!inherits(pval_vec, "try-error") && length(pval_vec) == 1 &&
                  ...
                  if (!is.na(bp_pvalue) && !is.na(var_ratio) && bp_pvalue < 0.05 &&
                    ...
                    reasons <- c(reasons, sprintf("Heteroscedasticity: BP_p=%.4f, var_ratio=%.2f",
                      bp_pvalue, var_ratio))
                  }
                }
            }
    ...
    # IMPORTANT (March 2026): Entropy and diversity metrics are MATHEMATICALLY
    # BOUNDED by definition (entropy in [0, log(m)]), not by measurement
    # artifacts.  Boundary clustering is therefore EXPECTED and NOT a
    # statistical problem.  Skip detection for entropy/diversity metrics to
    # avoid false positives.  Detection is retained for OTHER metrics where
    # boundaries indicate: - Detection limits / censoring - Measurement
    # artifacts - True data quality issues

    is_entropy_like <- tolower(value_col) %in% c("entropy", "diversity", "q_value",
        "tsallis")
    ...
    if (!is_entropy_like) {
        ...
        if (range_val > 0) {
            ...
            if (pct_near_bounds > 40) {
                ...
                reasons <- c(reasons, sprintf("Boundary clustering: %.1f%% within 10%% of bounds",
                  pct_near_bounds))
            }
        }
    } else {
        # For entropy/diversity metrics, explicitly note that boundary
        # clustering is expected and not treated as a special condition
        if (verbose) {
    ...
    # Bug #2 Fix (March 2026): Skewness should be independent condition (was:
    # if (abs(skewness_val) > 1 && characteristics$heteroscedastic)) Robust
    # median test should be available for ANY highly skewed data
...
.apply_art_kw <- function(data, value_col = "entropy", group_col = "q") {
    ...
    if (nrow(anova_res) >= 1) {
        ...
        return(list(statistic = f_stat, p_value = p_val, method = "ART-Kruskal-Wallis (heteroscedasticity-adjusted)"))
    } else {
        return(list(statistic = NA_real_, p_value = NA_real_, method = "test_failed"))
    }
}
...
#'
#' @noRd
.apply_quantile_test <- function(data, value_col = "entropy", group_col = "q", quantiles = c(0.25,
    0.5, 0.75)) {
    ...
    # Bug #1 Fix (March 2026): Test whether group QUANTILES differ (not raw
    # values) Strategy: For each quantile level, test if groups have different
    # quantile values using quantile regression or bootstrap CI on group
    # quantiles
    ...
    for (q_idx in seq_along(quantiles)) {
        ...
        # Create a pseudo-dataset where each group's value is its quantile Then
        # test using Kruskal-Wallis on ranks of those quantiles
        quant_data <- data.frame(q_value = as.numeric(group_quantiles), group = names(group_quantiles))
    ...
    # Combine p-values: use minimum with Bonferroni correction Safety check: if
    # all quantile tests failed, return NA
    if (all(is.na(quantile_pvals)) || all(is.na(quantile_stats))) {
        return(list(statistic = NA_real_, p_value = NA_real_, method = "Quantile-based test (insufficient data)"))
    }
    ...
    combined_pval <- pmin(1, min_pval * length(quantiles))

    return(list(statistic = max_stat, p_value = combined_pval, method = "Quantile-based test (boundary-adjusted)"))
}
...
.apply_robust_median_test <- function(data, value_col = "entropy", group_col = "q") {
    ...
    # Mood's median test 1. Compute grand median
    ...
    chisq_result <- tryCatch(chisq.test(contingency_table), error = function(e) NULL)

    if (!is.null(chisq_result)) {
        return(list(statistic = as.numeric(chisq_result$statistic), p_value = as.numeric(chisq_result$p.value),
...
#'
#' @noRd
.apply_friedman_test <- function(data, value_col = "entropy", group_col = "q", subject_col) {
    ...
    # Reshape to matrix: rows = blocks (subjects), columns = treatments
    # (q-values) NOTE: friedman.test() expects a matrix with no missing values
    friedman_matrix <- try({
        matrix_data <- xtabs(as.numeric(values) ~ blocks + treatments)
        matrix_data
    }, silent = TRUE)

    if (inherits(friedman_matrix, "try-error") || nrow(friedman_matrix) < 2 || ncol(friedman_matrix) <
        2) {
        return(list(statistic = NA_real_, p_value = NA_real_, method = "test_failed"))
    }
    ...
    if (any(is.na(friedman_matrix))) {
        ...
        return(list(statistic = NA_real_, p_value = NA_real_, method = "Friedman test (unbalanced blocks - skipped)"))
    }

    # Apply Friedman test
    friedman_result <- try(friedman.test(friedman_matrix), silent = TRUE)

    if (!inherits(friedman_result, "try-error")) {
        return(list(statistic = as.numeric(friedman_result$statistic), p_value = as.numeric(friedman_result$p.value),
            method = "Friedman test (paired)"))
    } else {
        return(list(statistic = NA_real_, p_value = NA_real_, method = "test_failed"))
    }
...
.apply_conditional_rank_test <- function(data, value_col = "entropy", group_col = "q",
    paired = FALSE, subject_col = NULL, verbose = FALSE) {

    # Priority 1: If paired design, use CONDITIONAL test selection
    if (paired && !is.null(subject_col) && subject_col %in% colnames(data)) {
        # NEW (March 2026): Conditional selection for paired tests Detect data
        # characteristics and select appropriate paired rank test
        selection <- .select_rank_test_paired(data, value_col, group_col, subject_col,
            verbose = verbose)

        # Apply selected paired test
        test_func <- switch(selection$test_selected, art_friedman = .apply_art_friedman,
            robust_friedman = .apply_robust_friedman, .apply_friedman_test  # Default: standard Friedman
)
        ...
        return(c(test_result, list(test_type = selection$test_selected, characteristics = characteristics_with_pairing)))
    }

    # Priority 2: If unpaired, use conditional selection logic Step 1: Detect
    # data characteristics
    selection <- .select_rank_test(data, value_col, group_col, verbose = verbose)

    # Step 2: Apply selected test
    test_func <- switch(selection$test_selected, art_kw = .apply_art_kw, quantile_test = .apply_quantile_test,
        robust_median_test = .apply_robust_median_test, function(d, v, g) {
    ...
    return(c(test_result, list(test_type = selection$test_selected, characteristics = selection$characteristics)))
}

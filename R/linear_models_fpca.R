...
.fpca_interaction <- function(mat, q_vals, sample_names, group_vec, g, min_obs = 10, subject = NULL, 
    ...
    df <- data.frame(
        entropy = as.numeric(mat[g, ]), q = as.numeric(q_vals), group = factor(group_vec),
    ...
    if (length(unique(na.omit(grp_vals))) < 2) return(NULL)
    
    subj_vals <- if (!is.null(subject)) df$subject[match(used_samples, df$sample_name)] else NULL
    ...
    if (regularization == "pca") {
        ...
        .fpca_regularization_method(mat_sub, grp_vals, subj_vals, g, regularization, weights)
    } else {
        NULL
    }
}
...
# Extracted from .fpca_interaction() to reduce function complexity
...
# 
# @param df Data frame with entropy, q, group, subject, sample_name columns
# @return Data frame with differenced values (or original if unpaired)
.apply_arima_differencing_fpca <- function(df) {
    ...
    for (subj in unique(df$subject)) {
        ...
        if (length(idx) >= 2) {
            ...
            df_list[[as.character(subj)]] <- data.frame(
...
# Build curve matrix: rows = samples, columns = ordered q-values
...
#
# @param entropy_vals Entropy values
# @param q_vals Q-values (q parameter)
# @param sample_names Sample identifiers
# @param min_obs Minimum observations per sample
# @return Curve matrix (samples × ordered q-values, with column indices respecting q-order) or NULL if insufficient data
...
# Impute missing values in curve matrix using column means
# Uses vectorized operation for efficiency (10-20x faster than row-by-row)
# 
# @param curve_mat Curve matrix with potential NA values
# @return Imputed curve matrix
...
# Aggregate values by subject (compute mean across q-values within each subject)
# Preparation for paired statistical tests
# 
# @param values Numeric values to aggregate
# @param group_vec Group assignments
# @param subject_vec Subject identifiers
# @param group_id Specific group to extract
# @return Aggregated values per subject
...
# Perform paired or unpaired t-test on PC values
# Automatically selects test based on subject information
# 
...
#
# @param pc_vals PC scores for all samples
# @param grp_vals Group assignments
# @param subj_vals Subject identifiers (NULL for unpaired)
# @param g1 First group value
# @param g2 Second group value
# @return P-value from test (or NA if test fails)
.test_pc_groupdiff <- function(pc_vals, grp_vals, subj_vals, g1, g2) {
    ...
    if (!is.null(subj_vals)) {
        ...
        if (length(pc_g1_subj) >= 2 && length(pc_g2_subj) >= 2 && 
            !anyNA(pc_g1_subj) && !anyNA(pc_g2_subj)) {
...
#
# @param pca PCA result object (from prcomp)
# @return Number of PCs to test
.select_npc <- function(pca) {
    cumsum_var <- cumsum(pca$sdev^2) / sum(pca$sdev^2)
    var_threshold <- 0.80  # Explains 80% of variance
...
# Returns adjusted p-value from multiple PCs tested
# 
# Multiple testing correction: Apply Benjamini-Hochberg (BH) correction for multiple testing.
# Rationale: PCs are orthogonal by construction but testing across multiple PCs introduces
# multiple comparisons problem. BH controls False Discovery Rate (FDR) which is more
# appropriate than FWER (Bonferroni) for exploratory testing, especially when PCs capture
# inter-related aspects of the same phenomenon (curve shape differences).
# BH is less conservative than Bonferroni and accounts for structure in test dependency.
#
# @param pca PCA object
# @param grp_vals Group assignments
# @param subj_vals Subject identifiers (NULL for unpaired)
# @return List with p_interaction (min adjusted), n_pcs_tested, min_pc_pvalue
.test_all_pcs <- function(pca, grp_vals, subj_vals) {
    ...
    for (pc_idx in seq_len(n_pc_use)) {
        ...
        pc_pvals[pc_idx] <- .test_pc_groupdiff(pc_vals, grp_vals, subj_vals, g1, g2)
    }
    ...
    p_interaction <- min(p_interaction, 1.0)
    
    list(
        p_interaction = p_interaction,
        n_pcs_tested = n_pc_use,
        min_pc_pvalue = if (length(pc_pvals_valid) > 0) min(pc_pvals_valid) else NA_real_
    )
}

# Fit PCA-based curve analysis
# Tests multiple PCs with BH correction for group differences
# 
...
# - PC2, PC3 capture secondary shape variations
#
# @param mat_sub Curve matrix (samples × sorted q-values)
# @param grp_vals Group assignments
# @param subj_vals Subject identifiers (NULL for unpaired)
# @param g Gene identifier
# @param weights Optional weights parameter
# @return Data frame with gene, p_interaction, n_pcs_tested, min_pc_pvalue
.fpca_pca_method <- function(mat_sub, grp_vals, subj_vals, g, weights = NULL) {
    ...
    if (inherits(pca, "try-error") || ncol(pca$x) < 1) return(NULL)
    
    result_pc <- .test_all_pcs(pca, grp_vals, subj_vals)
    if (is.null(result_pc$p_interaction) || is.na(result_pc$p_interaction)) return(NULL)
    
    data.frame(
        gene = g, p_interaction = result_pc$p_interaction, 
        n_pcs_tested = result_pc$n_pcs_tested,
        min_pc_pvalue = result_pc$min_pc_pvalue, 
        slope_diff = NA_real_, ci_weighted = !is.null(weights), 
        stringsAsFactors = FALSE
    )
}

# Fit LASSO/ElasticNet regression on curves
# Tests group difference in predicted probabilities respecting q-value ordering
# 
...
# @param subj_vals Subject identifiers (NULL for unpaired)
# @param g Gene identifier
# @param regularization "lasso" or "elasticnet"
# @param weights Optional weights parameter
# @return Data frame with gene, p_interaction, slope_diff
.fpca_regularization_method <- function(mat_sub, grp_vals, subj_vals, g, 
                                        regularization = "lasso", weights = NULL) {
    if (!requireNamespace("glmnet", quietly = TRUE)) {
        stop("Package 'glmnet' is required for regularization methods")
    }
    
    grp_numeric <- as.numeric(factor(grp_vals)) - 1
    alpha_val <- if (regularization == "lasso") 1 else 0.5
    
    cv_fit <- try(glmnet::cv.glmnet(x = mat_sub, y = grp_numeric, family = "binomial", 
                                    alpha = alpha_val, nfolds = min(5, nrow(mat_sub) - 1), 
                                    standardize = TRUE), silent = TRUE)
    if (inherits(cv_fit, "try-error")) return(NULL)
    
    pred_probs <- try(stats::predict(cv_fit, newx = mat_sub, s = "lambda.min", 
                                    type = "response"), silent = TRUE)
    if (inherits(pred_probs, "try-error") || is.null(pred_probs)) return(NULL)
    ...
    data.frame(gene = g, p_interaction = pval, slope_diff = NA_real_,

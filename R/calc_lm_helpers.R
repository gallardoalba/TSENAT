...
.tsenat_report_fit_summary <- function(res, verbose = TRUE) {
  if (verbose && "fit_method" %in% colnames(res)) {
    ...
    if ("singular" %in% colnames(res)) {
      ...
      if (n_sing > 0)
        message(sprintf("[calculate_lm_interaction] singular fits detected: %d/%d genes", n_sing, total_genes))
    }
  }
}
...
.tsenat_gam_interaction <- function(df, q_vals, g, min_obs = 10) {
  ...
  if (inherits(fit_null, "try-error") || inherits(fit_alt, "try-error")) return(NULL)
  an <- try(mgcv::anova.gam(fit_null, fit_alt, test = "F"), silent = TRUE)
  if (inherits(an, "try-error")) return(NULL)
  p_interaction <- NA_real_
  if (nrow(an) >= 2) {
    if ("Pr(F)" %in% colnames(an)) p_interaction <- an[2, "Pr(F)"]
    else if ("Pr(>F)" %in% colnames(an)) p_interaction <- an[2, "Pr(>F)"]
...
.tsenat_fpca_interaction <- function(mat, q_vals, sample_names, group_vec, g, min_obs = 10) {
  ...
  if (inherits(t_res, "try-error")) return(NULL)

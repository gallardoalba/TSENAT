...
plot_multi_gene_q_spectrum_s4 <- function(eff_res = NULL, lm_res = NULL, divergence_results_se = NULL,
    ...
    if (methods::is(eff_res, "TSENATAnalysis")) {
        ...
        # For S4 mode, set eff_res to NULL so we use lm_res +
        # divergence_results_se
        eff_res <- NULL
    }

    # ============================================================================
    # Step 1: Extract gene data from either eff_res or (lm_res +
    # divergence_results_se)
    # ============================================================================
    ...
    if (!is.null(eff_res) && is.list(eff_res)) {
        if (!is.null(eff_res$interaction_results) && nrow(eff_res$interaction_results) >
            ...
            if (has_gene && has_per_q && has_p_any) {
                # Sort by significance (lowest p-value first = most
                # significant)
        ...
        if (verbose) {
            ...
            if (!is.null(eff_res)) {
                ...
                if (!is.null(eff_res$interaction_results)) {
                  ...
                  message("  - Has 'per_q_pattern' column:", "per_q_pattern" %in%
                    colnames(eff_res$interaction_results))
                }
            }
    ...
    if (verbose)
        message(sprintf("Plotting %d genes in %d-column grid", n_genes_actual, ncol))
    ...
    for (i in seq_along(genes_to_plot)) {
        ...
        tryCatch({
            ...
            if (length(per_q_vals) == 0 || all(is.na(per_q_vals))) {
                if (verbose)
                  message(sprintf("  Skipping %s: no valid per-q values", gene_name))
                next
            }
            ...
            p <- ggplot2::ggplot(plot_df, ggplot2::aes(x = q, y = divergence)) +
                ...
                color = "gray60", linewidth = 0.8, alpha = 0.7) + ggplot2::labs(title = sprintf("%s",
                gene_name), subtitle = sprintf("adj p = %.2e", adj_p), x = "q (Tsallis parameter)",
                y = "Tsallis Divergence D[q]") + ggplot2::theme(plot.subtitle = ggplot2::element_text(hjust = 0.5,
                size = 10, color = "gray40", margin = ggplot2::margin(b = 8)), plot.margin = ggplot2::margin(t = 8,
                b = 8, l = 6, r = 6), panel.grid.major = ggplot2::element_line(color = "gray92",
            ...
            if (verbose)
                message(sprintf("  Failed to plot %s: %s", gene_name, e$message))
        })
    }
    ...
    for (row_idx in seq_len(nrow)) {
        ...
        # Additional safety: filter row_plots to ensure all are valid ggplot
        # objects
    ...
    combined_plot <- Reduce(function(x, y) x/y, layout_plots) + patchwork::plot_layout(heights = c(rep(c(1,

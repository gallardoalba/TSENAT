...
.bootstrap_divergence <- function(x = NULL, y = NULL, se = NULL, res = NULL, top_n = 1,
    ...
    if (!is.null(se) && !is.null(res)) {
        ...
        # Get top gene by rank (assuming res is sorted by p-value or
        # significance)
        gene_idx <- top_n
    ...
    # Suppress nboot < 100 warning during testing if (nboot < 100 &&
    # !isTRUE(getOption('TSENAT.suppress_nboot_warning'))) { warning('nboot <
    # 100 may produce unstable confidence intervals') }
    ...
    if (length(q) > 1) {
        results_list <- lapply(q, function(qi) {
            .bootstrap_divergence(x = x, y = y, se = NULL, res = NULL, q = qi, norm = norm,
                ...
                seed = seed, gene_name = gene_name, verbose = FALSE, paired = paired,
        ...
        if (verbose && !is.null(gene_name)) {
            ...
            for (qi in seq_along(q)) {
                ...
                message(sprintf("q = %.2f: D_q = %.4f [%.4f, %.4f] (width=%.4f)",
                  q[qi], r$estimate, r$lower_ci, r$upper_ci, r$upper_ci - r$lower_ci))
            }
            message("")
        }
    ...
    if (isTRUE(paired) && !is.null(se)) {
        # PAIRED BOOTSTRAP: Resample pairs as units while maintaining pairing
        # structure Extract pair_id information from colData
        ...
        if (is.null(pair_ids)) {
            stop("paired=TRUE but no pair_id column found in colData. ", "Provide pair_id_col or ensure colData contains pairing information.")
        }
        ...
        # Convert pairs to a numeric pair_ids vector for C++ bootstrap Map
        # original pair_ids to sequential indices 1, 2, ..., n_pairs
        ...
        # Speedup: 10-15× via multinomial resampling in C++ Strategy: Delegate
        # pair resampling to C++ function, which: - Extracts pair structure
        # from pair_ids_numeric - Resamples pairs as units with replacement -
        ...
        bootstrap_divs <- divergence_bootstrap_paired_cpp_wrapper(x = as.numeric(x),
            y = as.numeric(y), pair_ids = pair_ids_numeric, nboot = nboot, q = q,
            pseudocount = pseudocount, log_base = log_base)
    } else {
        ...
        bootstrap_divs <- divergence_bootstrap_compute_cpp_wrapper(x = as.numeric(x),
            y = as.numeric(y), nboot = nboot, q = q, pseudocount = pseudocount, log_base = log_base,
            paired = FALSE)
    }
    ...
    result <- list(estimate = estimate, lower_ci = as.numeric(lower_ci), upper_ci = as.numeric(upper_ci),

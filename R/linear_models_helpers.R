...
.report_fit_summary <- function(res, verbose = TRUE) {
    if (verbose && "fit_method" %in% colnames(res)) {
        ...
        if ("singular" %in% colnames(res)) {
            ...
            if (n_sing > 0) {
                message(sprintf("[calculate_lm_interaction]   Singular fits (collinear effects): %d genes",
                  n_sing))
            }
        }
...
#' @noRd
.validate_lm_interaction_input <- function(method, pvalue, corstr, regularization,
...
.parse_sample_metadata <- function(se, condition_col, assay_name, verbose) {
    ...
    if (verbose) {
        message("[calculate_lm_interaction] parsed samples and groups: ", length(unique(sample_names)),
...
.fit_all_genes <- function(mat, se, metadata, method, pvalue, subject_col, paired,
    ...
    if (!"ci_weighted" %in% colnames(res)) {
        ...
        if (verbose) {
            warning("[calculate_lm_interaction] ci_weighted column was ", "missing; added as NAs. This suggests a method helper ",
    ...
    if (storey) {
        if (requireNamespace("fdrtool", quietly = TRUE)) {
            tryCatch({
                ...
                if (verbose) {
                  message("[calculate_lm_interaction] Applied Storey ", "adaptive FDR pi0 correction to ",
                    multicorr, " p-values")
                }
            }, error = function(e) {
                if (verbose) {
                  message("[calculate_lm_interaction] Storey adjustment ", "failed: ",
                    conditionMessage(e))
                }
            })
        } else if (verbose) {
            message("[calculate_lm_interaction] fdrtool package not ", "available for Storey (install with: ",
...
.map_gene_annotations <- function(res, se, verbose) {
    ...
    if (verbose) {
        message("[calculate_lm_interaction] Gene annotations: ", paste(colnames(rd),
            collapse = ", "))
    }

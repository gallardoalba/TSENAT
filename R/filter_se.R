...
#' - S063 (2023): Power and Sample Size for RNA-seq - supports power-driven thresholds
#' - C022 (2017): Generalized models - validates combined effect size + testing
...
filter_se_by_effect_size <- function(se, interaction_results, eta2_threshold = 0.02,
    ...
    if (!is.data.frame(interaction_results)) {
        stop("'interaction_results' must be a data frame from detect_q_gene_interactions()", 
             call. = FALSE)
    }
    
    if (!("gene" %in% colnames(interaction_results))) {
        stop("'interaction_results' must contain 'gene' column", call. = FALSE)
    }
    
    # Find effect size column (may be named 'eta2' or 'effect_size_eta2')
    eta2_col <- if ("effect_size_eta2" %in% colnames(interaction_results)) {
        "effect_size_eta2"
    } else if ("eta2" %in% colnames(interaction_results)) {
        "eta2"
    } else {
        stop("'interaction_results' must contain 'effect_size_eta2' or 'eta2' column", 
             call. = FALSE)
    }
    
    if (!is.numeric(eta2_threshold) || eta2_threshold < 0 || eta2_threshold > 1) {
        stop("'eta2_threshold' must be a numeric value between 0 and 1", call. = FALSE)
    }
    
    # Identify genes exceeding threshold
    genes_above_threshold <- interaction_results$gene[
        interaction_results[[eta2_col]] > eta2_threshold & 
        !is.na(interaction_results[[eta2_col]])
    ]
    
    # Filter SE to keep only these genes
    genes_in_se <- rownames(se)
    genes_to_keep <- genes_in_se[genes_in_se %in% genes_above_threshold]
    
    before <- length(genes_in_se)
    after <- length(genes_to_keep)
    removed <- setdiff(genes_in_se, genes_to_keep)
    ...
    if (verbose) {
        ...
        # Show effect size statistics
        effect_sizes <- interaction_results[[eta2_col]][interaction_results$gene %in% genes_in_se]
    ...
    if (after == 0) {
        warning("Effect size filtering removed all genes; returning empty SummarizedExperiment.",
                call. = FALSE)
    }
    ...
    new_assays <- S4Vectors::SimpleList(lapply(assays_list, function(a) {
        if (is.matrix(a) || is.data.frame(a)) {
            as.matrix(a)[genes_to_keep, , drop = FALSE]
        } else {
            a
        }
    }))
    ...
    rd <- NULL
    if (nrow(SummarizedExperiment::rowData(se)) > 0) {
        rd <- SummarizedExperiment::rowData(se)[genes_to_keep, , drop = FALSE]
    }
    
    # Update metadata
    md <- S4Vectors::metadata(se)
    
    # Preserve readcounts and tx2gene if present
    if (!is.null(md$readcounts) && is.matrix(md$readcounts)) {
        md$readcounts <- as.matrix(md$readcounts)[genes_to_keep, , drop = FALSE]
    }
    if (!is.null(md$tx2gene) && is.data.frame(md$tx2gene)) {
        txmap <- md$tx2gene
        txcol <- colnames(txmap)[1]
        md$tx2gene <- txmap[txmap[[txcol]] %in% genes_to_keep, , drop = FALSE]
    }
    ...
    new_se <- SummarizedExperiment::SummarizedExperiment(
        ...
        metadata = c(md, list(
            effect_size_filter = list(
                threshold = eta2_threshold,
                genes_before = before,
                genes_after = after,
